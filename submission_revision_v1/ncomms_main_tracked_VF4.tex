...
%DIF DEL original_submission.tex   Tue May 19 16:55:37 2026
%DIF ADD revised_VF4.tex           Tue May 19 16:57:18 2026
%DIF < % Nature Machine Intelligence — Article
%DIF < % "Same Prompt, Different Answer: Exposing the Reproducibility Illusion
%DIF < %  in Large Language Model APIs"
%DIF -------
% Nature Communications — Article %DIF > 
% "Same Prompt, Different Answer: Hidden Non-Determinism in LLM APIs %DIF > 
%  Undermines Scientific Reproducibility" %DIF > 
%DIF -------
%
...
%DIF -------
\raggedbottom
%DIF < \unnumbered% Nature MI uses unnumbered section heads
%DIF -------
\unnumbered% Nature Communications uses unnumbered section heads %DIF > 
...
\title[Same Prompt, Different Answer]{Same Prompt, Different Answer: \DIFdelbegin \DIFdel{Exposing the Reproducibility Illusion }\DIFdelend \DIFaddbegin \DIFadd{Hidden Non-Determinism }\DIFaddend in \DIFdelbegin \DIFdel{Large Language Model }\DIFdelend \DIFaddbegin \DIFadd{LLM }\DIFaddend APIs \DIFaddbegin \DIFadd{Undermines Scientific Reproducibility}\DIFaddend }
...
\DIFaddbegin \author[1]{\fnm{Hugo Valadares} \sur{Siqueira}\DIFadd{~}\orcidlink{0000-0002-1278-4602}}

\DIFaddend \author[2]{\fnm{Eduardo Tadeu} \sur{Bacalhau}\DIFaddbegin \DIFadd{~}\orcidlink{0000-0002-3936-0375}\DIFaddend }

\author[3]{\fnm{Anibal Tavares} \spfx{de} \sur{Azevedo}\DIFaddbegin \DIFadd{~}\orcidlink{0000-0003-1678-7795}\DIFaddend }

\author[1]{\DIFdelbegin %DIFDELCMD < \fnm{Hugo Valadares} \sur{Siqueira}}
%DIFDELCMD < 

%DIFDELCMD < \author[1]{%%%
\DIFdelend \fnm{Yara} \spfx{de Souza} \sur{Tadano}\DIFaddbegin \DIFadd{~}\orcidlink{0000-0002-3975-3419}\DIFaddend }
...
\DIFdelbegin %DIFDELCMD < \abstract{The same prompt sent twice to a large language model API under documented ``deterministic'' settings can return different answers, yet this variation is invisible to users.
%DIFDELCMD < Here we report 4,104 controlled experiments across eight models and five API providers showing that, under temperature-zero greedy decoding with fixed seeds, API-served models reproduce their own outputs only 22.1\% of the time (exact match rate), while locally deployed models achieve 95.6\%, a gap exceeding four-fold.
%DIFDELCMD < The non-determinism persists in multi-turn and retrieval-augmented generation workflows, where one model produces zero exact matches across 50 runs, yet remains hidden because outputs are semantically equivalent (BERTScore F1~$>$~0.97).
%DIFDELCMD < A quasi-isolation experiment identifies production infrastructure complexity, rather than cloud deployment itself, as the driver.
%DIFDELCMD < We provide a lightweight provenance protocol ($<$1\% overhead) that makes this variation detectable, raising a reliability concern for LLMs in medicine, physical sciences, and automated data analysis.}
%DIFDELCMD < %%%
...
They encode clinical knowledge\cite{singhal2023large}, assist diagnostic workflows\cite{thirunavukarasu2023large}, accelerate literature review and data extraction\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{noy2023experimental}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{birhane2023science}}\hskip0pt%DIFAUXCMD
}\DIFaddend , and are increasingly integrated into peer review at scale\cite{thakkar2026llm}.
...
Our experiments show that this guarantee does not hold\DIFdelbegin \DIFdel{in practice}\DIFdelend .
...
\DIFaddbegin \DIFadd{A central conceptual point underlies our analysis: when a researcher queries an LLM through an API, the experimental object is not a model in the abstract sense but a }\emph{\DIFadd{deployment stack}}\DIFadd{, the tuple (model weights, provider, serving infrastructure, API layer).
...
\DIFaddend Here, we make \DIFdelbegin \DIFdel{four }\DIFdelend \DIFaddbegin \DIFadd{five }\DIFaddend contributions.
...
Third, we demonstrate that cloud deployment \emph{per se} does not cause non-determinism: the same LLaMA~3 8B \DIFdelbegin \DIFdel{architecture served via }\DIFdelend \DIFaddbegin \DIFadd{weights served via two stacks (local Ollama versus }\DIFaddend Together AI's cloud endpoint\DIFdelbegin \DIFdel{achieves near-local }\DIFdelend \DIFaddbegin \DIFadd{) achieve near-identical }\DIFaddend exact match rates\DIFdelbegin \DIFdel{, implicating }\DIFdelend \DIFaddbegin \DIFadd{. This implicates }\DIFaddend production-infrastructure \DIFdelbegin \DIFdel{complexity (tensor }\DIFdelend \DIFaddbegin \DIFadd{complexity---tensor }\DIFaddend parallelism, speculative decoding, dynamic \DIFdelbegin \DIFdel{batching) rather }\DIFdelend \DIFaddbegin \DIFadd{batching---rather }\DIFaddend than the cloud medium \DIFaddbegin \DIFadd{itself, and confirms that the deployment stack, not the model in isolation, is the carrier of variation}\DIFaddend .
...
\DIFdelbegin %DIFDELCMD < \subsection{API-based models fail to reproduce outputs under deterministic settings}%%%
\DIFdelend \DIFaddbegin \subsection{API-served deployment stacks fail to reproduce outputs under deterministic settings}\DIFaddend \label{subsec:greedy}

We evaluated \DIFdelbegin \DIFdel{eight models }\DIFdelend \DIFaddbegin \DIFadd{nine deployment stacks (Methods~\ref{subsec:models}) }\DIFaddend across two core tasks, structured extraction (JSON output) and scientific \DIFdelbegin \DIFdel{summarization }\DIFdelend \DIFaddbegin \DIFadd{summarisation }\DIFaddend (free text), under greedy decoding (temperature~$=$~0) with fixed random seeds, the configuration that API documentation presents as deterministic.
...
Perplexity Sonar falls further to EMR~$=$~0.100 for extraction and 0.010 for \DIFdelbegin \DIFdel{summarization.
Averaging across all four tasks (including multi-turn and RAG; see Fig.~\ref{fig:multiturn}), the }\DIFdelend \DIFaddbegin \DIFadd{summarisation.
The }\DIFaddend overall local average EMR is 0.956 versus 0.221 for API models \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(averaged across GPT-4 and Claude Sonnet 4.5, the two API stacks with full Tasks~1--4 coverage; the four-stack average across all API entries in Table~1 is 0.319), }\DIFaddend meaning that fewer than one in four API output pairs are identical under conditions documented as deterministic.
This gap survives Holm--Bonferroni correction across 68 hypothesis tests (51 significant at $\alpha$~$=$~0.05; Supplementary Section~S9) and is confirmed by large Cliff's delta effect sizes ($\delta$~$=$~0.784--0.896\DIFdelbegin \DIFdel{; Extended Data Table~2}\DIFdelend ), paired $t$-test (Cohen's $d$~$>$~1.6), and a balanced 10-abstract subsample analysis that controls for sample-size differences between models (local EMR~$=$~0.953 vs.\ API EMR~$=$~0.304, 3.1$\times$ gap; Extended Data Table~4).
...
\begin{table}[h]
\caption{\textbf{\DIFaddFL{At-a-glance mechanism activation by deployment-stack class.}} \DIFaddFL{Reads alongside the cloud-vs-infrastructure inference argument: Together AI strips the production-serving cluster while retaining cloud-network and FP arithmetic factors, isolating production-serving complexity as the residual driver of variation in closed APIs. ``mitigated'' = single-GPU integer-arithmetic execution or quasi-isolation by design; ``active'' = documented or strongly likely from public information. See Methods Table~\ref{tab:mechanisms} for the per-mechanism, per-stack breakdown.}}\label{tab:mech_summary}
\footnotesize
\centering
...
\end{tabular}
\\[4pt]
{\scriptsize \DIFaddFL{\textsuperscript{a}FP non-associativity, BF16/FP16 mixed-precision; \textsuperscript{b}latency, multi-tenancy, request routing; \textsuperscript{c}tensor parallelism, FlashAttention, dynamic batching, speculative decoding; \textsuperscript{*}GPT-4, Claude, Gemini, DeepSeek, Perplexity.}}
\end{table}
...
\DIFaddbegin \subsection{Coding and math reasoning extend the reproducibility gap to high-stakes outputs}\label{subsec:codemath}
...
This three-level dissociation (low bitwise identity, moderate surface divergence, high semantic preservation) defines ``hidden'' non-determinism: same meaning, different words\DIFdelbegin \DIFdel{(Extended Data Table~1; Extended Data Fig.
~1).
}\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend A researcher reading two outputs from the same prompt would judge them equivalent; only systematic comparison reveals that they are textually distinct.
...
API models, however, start from an already-low baseline at $t$~$=$~0 and show a more complex pattern that violates the expected monotonicity\DIFdelbegin \DIFdel{(Extended Data Table~3)}\DIFdelend .
...
\DIFaddbegin \subsection{Applied impact in evidence synthesis: an out-of-AI/ML probe}\label{subsec:pubmed}
...
\begin{figure}[t]
  ...
  \caption{\DIFdelbeginFL \textbf{\DIFdelFL{Exact match rates under greedy decoding reveal a four-fold local--API gap.}}
  %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Exact match rates per deployment stack under greedy decoding reveal a four-fold local--API gap.}}
  \DIFaddendFL Heatmap of EMR \DIFdelbeginFL \DIFdelFL{for model deployments }\DIFdelendFL across extraction and \DIFdelbeginFL \DIFdelFL{summarization }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{summarisation }\DIFaddendFL tasks under temperature~$=$~\DIFdelbeginFL \DIFdelFL{0.
  Local models }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0
  ...
  Local stacks }\DIFaddendFL achieve EMR~$\geq$~0.840, with \DIFdelbeginFL \DIFdelFL{Gemma~2 9B }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{L-Gemma2 }\DIFaddendFL at a perfect 1.000.
  API-served \DIFdelbeginFL \DIFdelFL{models (bottom; red) }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{stacks }\DIFaddendFL range from 0.800 (\DIFdelbeginFL \DIFdelFL{DeepSeek}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{API-DeepSeek}\DIFaddendFL ) to 0.010 (\DIFdelbeginFL \DIFdelFL{Perplexity}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{API-Perplexity}\DIFaddendFL ).
  All values are computed under each \DIFdelbeginFL \DIFdelFL{model}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{stack}\DIFaddendFL 's representative greedy condition (C1 for local \DIFdelbeginFL \DIFdelFL{models }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{stacks, C-LLaMA3, API-Claude, API-DeepSeek }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{Claude}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{API-Perplexity}\DIFaddendFL ; C2 for \DIFdelbeginFL \DIFdelFL{GPT-4}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{API-GPT4 because the OpenAI seed is advisory}\DIFaddendFL ; see Methods).
  \DIFdelbeginFL \DIFdelFL{Full results for all eight models appear in Table~\ref{tab:main}.
  }\DIFdelendFL }\label{fig:heatmap}
\end{figure}

%% --- Table 1: EMR with CIs (full width) ---
\begin{table*}[t]
\caption{\DIFdelbeginFL \textbf{\DIFdelFL{Exact match rate under greedy decoding with 95\% bootstrap confidence intervals.}}
%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Exact match rate per deployment stack under greedy decoding, with 95\% bootstrap confidence intervals.}}
\DIFaddFL{Each row identifies a stack by its weights and provider/infrastructure class; full tuples (weights, provider, serving infrastructure, API layer) are listed in Methods~\ref{subsec:models}.
}\DIFaddendFL For local \DIFdelbeginFL \DIFdelFL{models}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{stacks}\DIFaddendFL , values reflect the fixed-seed condition (C1); for GPT-4, the variable-seed greedy condition (C2); for Claude, C1 (no seed parameter supported).
...
\begin{tabular}{@{}llccc@{}}
\toprule
\textbf{\DIFdelbeginFL \DIFdelFL{Model}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Deployment stack (weights)}\DIFaddendFL } & \textbf{\DIFdelbeginFL \DIFdelFL{Deployment}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Class}\DIFaddendFL } & \textbf{$n$} & \textbf{Extraction EMR} & \textbf{Summarization EMR} \\
\midrule
...
\midrule
\multicolumn{2}{l}{\DIFdelbeginFL \textit{\DIFdelFL{Local average}}%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textit{\DIFaddFL{Local-stack average}}\DIFaddendFL } & & 0.982 & 0.929 \\
\multicolumn{2}{l}{\DIFdelbeginFL \textit{\DIFdelFL{API average}}%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textit{\DIFaddFL{API-stack average}}\DIFaddendFL }   & & 0.383 & 0.255 \\
\botrule
\end{tabular}
\normalsize
\end{table*}
...
\begin{figure*}[t]
  ...
  \caption{\DIFdelbeginFL \textbf{\DIFdelFL{Complex interaction regimes amplify the local--API reproducibility gap.}}
  %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Complex interaction regimes amplify the local--API reproducibility gap across deployment stacks.}}
  \DIFaddendFL EMR under greedy decoding (C1, $t$~$=$~0) for \DIFdelbeginFL \DIFdelFL{five models }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the local stacks (Gemma~2, Mistral, LLaMA~3) and four API stacks (Claude Sonnet~4.5, Gemini~2.5 Pro, gpt-4o, and DeepSeek Chat) }\DIFaddendFL across four scenarios:
  ...
  Local \DIFdelbeginFL \DIFdelFL{models (Gemma~2, Mistral, LLaMA~3) }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{stacks }\DIFaddendFL maintain EMR~$\geq$~0.880 across all scenarios.
  \DIFdelbeginFL \DIFdelFL{Both }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{All four }\DIFaddendFL API \DIFdelbeginFL \DIFdelFL{models }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{stacks exhibit low EMR on multi-turn refinement
  ...
  with \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL Claude \DIFaddbeginFL \DIFaddFL{stack }\DIFaddendFL achieving 0.000 for RAG (50 runs, zero exact matches).
...
\begin{figure*}[t]
  ...
  \textbf{Centre:} EMR under greedy decoding for eight \DIFdelbeginFL \DIFdelFL{model deployments}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{deployment stacks}\DIFaddendFL ,
  illustrating the local--API reproducibility gap
  (local \DIFdelbeginFL \DIFdelFL{models }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{stacks }\DIFaddendFL green, EMR~$\geq$~0.840; API \DIFdelbeginFL \DIFdelFL{models }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{stacks }\DIFaddendFL red, EMR~$\leq$~0.800).
  \textbf{Right:} Key statistics: \DIFdelbeginFL \DIFdelFL{4}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{7}{\DIFaddendFL ,\DIFdelbeginFL \DIFdelFL{104 }\DIFdelendFL \DIFaddbeginFL }\DIFaddFL{004 }\DIFaddendFL experiments, 9 \DIFdelbeginFL \DIFdelFL{deployments}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{deployment stacks}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{7 execution environments}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{6 task families}\DIFaddendFL ,
...
\DIFaddbegin \DIFadd{A direct consequence is that the }\emph{\DIFadd{deployment stack}}\DIFadd{, not the abstract model, is the carrier of variation in LLM-based research.
...
\DIFaddend These findings also have regulatory implications.
...
The provenance protocol we provide offers a practical path forward\DIFdelbegin \DIFdel{(Fig.
~\ref{fig:overview}).
}\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend Grounded in W3C PROV\cite{w3cprov2013} and adding less than 1\% overhead (Extended Data Table~6), it creates auditable records linking every output to its generation context.
Its key property is \emph{differential diagnosis}: when all hashes match except the output hash, the divergence is automatically attributed to the generation process, a result \DIFdelbegin \DIFdel{confirmed across all 4,104 }\DIFdelend \DIFaddbegin \DIFadd{verified by the protocol-minimality ablation across 1}{\DIFadd{,}}\DIFadd{864 }\DIFaddend runs (Supplementary Section~\DIFdelbegin \DIFdel{S4; Extended Data Fig.~2) }\DIFdelend \DIFaddbegin \DIFadd{S6) and consistent with the full 4}{\DIFadd{,}}\DIFadd{104-run analysis}\DIFaddend .
...
\DIFdelbegin \DIFdel{In summary, we have shown that the }\DIFdelend \DIFaddbegin \DIFadd{The Together AI quasi-isolation result also has scope limits that we wish to make explicit.
...
\DIFadd{Evidence from an applied domain confirms that the effects reported here have direct consequences for scientific conclusions.
In a companion study applying six LLMs to a 500-abstract corpus of epidemiological studies on fine particulate matter (PM\textsubscript{2.5}) and respiratory health\mbox{%DIFAUXCMD
\cite{rover2026evidence}}\hskip0pt%DIFAUXCMD
, we find that extraction EMR under identical deterministic settings drops to 0.050 (Claude), 0.150 (GPT-4.1), and 0.200 (Gemini~2.5 Pro) on the full 500-abstract corpus---consistent with, and in some cases worse than, the values reported here on the 10-abstract probe (where Claude EMR collapses further to 0.010 because the smaller corpus concentrates the within-abstract variance; the two values therefore quantify the same phenomenon at different sampling scales).
...
\DIFadd{The }\DIFaddend ``deterministic'' settings offered by major LLM API providers do not deliver determinism in practice, creating a hidden reproducibility gap that \DIFdelbegin \DIFdel{affects any study relying on API-served models.
The gap spans five independent }\DIFdelend \DIFaddbegin \DIFadd{spans five }\DIFaddend providers, persists across multi-turn and \DIFdelbegin \DIFdel{retrieval-augmented generation }\DIFdelend \DIFaddbegin \DIFadd{RAG }\DIFaddend workflows, and remains invisible \DIFdelbegin \DIFdel{to users }\DIFdelend because semantic equivalence \DIFdelbegin \DIFdel{masks textual divergence.
By running the same open-weight model locally and via a cloud API, we isolate the cause : }\DIFdelend \DIFaddbegin \DIFadd{under aggregate metrics masks textual---and sometimes substantive---divergence.
Our quasi-isolation experiment attributes the cause to }\DIFaddend production infrastructure complexity \DIFdelbegin \DIFdel{, not }\DIFdelend \DIFaddbegin \DIFadd{rather than }\DIFaddend cloud deployment itself\DIFdelbegin \DIFdel{, drives the non-determinism.
Our }\DIFdelend \DIFaddbegin \DIFadd{.
The }\DIFaddend lightweight provenance protocol \DIFaddbegin \DIFadd{we provide}\DIFaddend , grounded in W3C PROV and requiring less than 1\% overhead, \DIFdelbegin \DIFdel{offers a practical mechanism for making }\DIFdelend \DIFaddbegin \DIFadd{makes }\DIFaddend this variation visible, auditable, and attributable\DIFdelbegin \DIFdel{.
Together with the minimum reporting standard we propose, it provides }\DIFdelend \DIFaddbegin \DIFadd{, offering }\DIFaddend an actionable path toward reproducible LLM-based research.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{These findings raise a broader reliability concern.
Large language models are no longer confined to text generation; they are increasingly }\DIFdelend \DIFaddbegin \DIFadd{As large language models become }\DIFaddend embedded in clinical decision support, environmental monitoring, engineering design, \DIFdelbegin \DIFdel{automated data collection and analysis, and other domains where outputs directly inform decisions with consequences in the physical world.
In such settings, the }\DIFdelend \DIFaddbegin \DIFadd{and automated data analysis, the }\DIFaddend inability to reproduce \DIFdelbegin \DIFdel{or verify }\DIFdelend a model's output is not merely an academic inconvenience but a threat to the trustworthiness of \DIFdelbegin \DIFdel{the }\DIFdelend systems that depend on it.
\DIFdelbegin \DIFdel{Our results demonstrate that, without }\DIFdelend \DIFaddbegin \DIFadd{Without }\DIFaddend explicit provenance tracking, the reliability of any LLM-based pipeline built on API inference cannot be assumed; it must be measured, documented, and continuously monitored.
\DIFdelbegin \DIFdel{As the integration of AI into science and society accelerates, establishing the infrastructure for verifiable and reproducible model outputs is not a technical nicety but a scientific and societal necessity.
}\DIFdelend 
...
\DIFdelbegin %DIFDELCMD < \subsection{Models and infrastructure}%%%
\DIFdelend \DIFaddbegin \subsection{Unit of analysis: deployment stacks}\label{subsec:stacks}

\DIFadd{We define a }\emph{\DIFadd{deployment stack}} \DIFadd{as the tuple (model weights, provider, serving infrastructure, API layer).
...
We therefore report results per }\emph{\DIFadd{stack}} \DIFadd{rather than per }\emph{\DIFadd{model}}\DIFadd{.
...
\subsection{Deployment stacks evaluated}\DIFaddend \label{subsec:models}

We evaluate nine \DIFdelbegin \DIFdel{model deployments }\DIFdelend \DIFaddbegin \DIFadd{deployment stacks }\DIFaddend across three paradigms.
\DIFaddbegin \DIFadd{Each stack is identified by the tuple (model weights / provider / serving infrastructure / API layer).
}\DIFaddend 

\textbf{Local \DIFdelbegin \DIFdel{models}\DIFdelend \DIFaddbegin \DIFadd{stacks}\DIFaddend } (Ollama v0.15.5, Apple M4, 24\,GB unified memory, macOS~14.6, Python~3.14.3):
\DIFaddbegin \textbf{\DIFadd{L-LLaMA3}} \DIFadd{= (}\DIFaddend LLaMA~3 8B \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{grattafiori2024llama3} }\hskip0pt%DIFAUXCMD
(}\DIFdelend Q4\_0\DIFdelbegin \DIFdel{),
Mistral}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{grattafiori2024llama3} }\hskip0pt%DIFAUXCMD
/ local Ollama / single-GPU Apple M4 / Ollama }\texttt{\DIFadd{/api/generate}}\DIFadd{);
}\textbf{\DIFadd{L-Mistral}} \DIFadd{= (Mistral~}\DIFaddend 7B \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{jiang2023mistral} }\hskip0pt%DIFAUXCMD
(}\DIFdelend Q4\_0\DIFdelbegin \DIFdel{),
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{jiang2023mistral} }\hskip0pt%DIFAUXCMD
/ local Ollama / single-GPU Apple M4 / Ollama }\texttt{\DIFadd{/api/generate}}\DIFadd{);
}\textbf{\DIFadd{L-Gemma2}} \DIFadd{= (}\DIFaddend Gemma~2 9B \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{team2024gemma} }\hskip0pt%DIFAUXCMD
(}\DIFdelend Q4\_0\DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{team2024gemma} }\hskip0pt%DIFAUXCMD
/ local Ollama / single-GPU Apple M4 / Ollama }\texttt{\DIFadd{/api/generate}}\DIFaddend ).
Weights hashes recorded via the Ollama API.

\textbf{API-served \DIFdelbegin \DIFdel{models}%DIFDELCMD < \MBLOCKRIGHTBRACE%%%
\DIFdel{:
GPT-4 }\DIFdelend \DIFaddbegin \DIFadd{stacks}} \DIFadd{(full stack opaque; reference local pair available only for L-LLaMA3):
}\textbf{\DIFadd{API-GPT4}} \DIFadd{= }\DIFaddend (\texttt{gpt-4-0613}\DIFdelbegin \DIFdel{)\mbox{%DIFAUXCMD
\cite{achiam2023gpt4} }\hskip0pt%DIFAUXCMD
via OpenAI API;
Claude Sonnet~4.5 }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{achiam2023gpt4} }\hskip0pt%DIFAUXCMD
/ OpenAI / production cluster / Chat Completions API);
}\textbf{\DIFadd{API-Claude}} \DIFadd{= }\DIFaddend (\texttt{claude-sonnet-4-5-20250929}\DIFdelbegin \DIFdel{)\mbox{%DIFAUXCMD
\cite{anthropic2024claude} }\hskip0pt%DIFAUXCMD
via Anthropic API (urllib, no SDK);
Gemini~2.5 Pro }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{anthropic2024claude} }\hskip0pt%DIFAUXCMD
/ Anthropic / production cluster / Messages API via urllib);
}\textbf{\DIFadd{API-Gemini}} \DIFadd{= }\DIFaddend (\texttt{gemini-2.5-pro-preview-05-06}\DIFdelbegin \DIFdel{)\mbox{%DIFAUXCMD
\cite{reid2024gemini15} }\hskip0pt%DIFAUXCMD
via Google }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{reid2024gemini15} }\hskip0pt%DIFAUXCMD
/ Google / production cluster / }\DIFaddend AI Studio REST API \DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{via }\DIFaddend urllib);
\DIFaddbegin \textbf{\DIFadd{API-DeepSeek}} \DIFadd{= (DeepSeek Chat / }\DIFaddend DeepSeek \DIFdelbegin \DIFdel{Chat via }\DIFdelend \DIFaddbegin \DIFadd{/ production cluster / }\DIFaddend OpenAI-compatible API\DIFdelbegin \DIFdel{;
Perplexity Sonar via Perplexity API}\DIFdelend \DIFaddbegin \DIFadd{);
}\textbf{\DIFadd{API-Perplexity}} \DIFadd{= (Sonar / Perplexity / production cluster + retrieval / Perplexity API)}\DIFaddend .

\textbf{Quasi-isolation probe \DIFaddbegin \DIFadd{stack}\DIFaddend }:
\DIFaddbegin \textbf{\DIFadd{C-LLaMA3}} \DIFadd{= (}\DIFaddend LLaMA~3 8B \DIFdelbegin \DIFdel{via Together AI }\DIFdelend \DIFaddbegin \DIFadd{Q4\_0 }\DIFaddend (INT4\DIFdelbegin \DIFdel{, same architecture as local, }\texttt{\DIFdel{meta-llama/Llama-3-8b-chat-hf}}%DIFAUXCMD
\DIFdel{)}\DIFdelend \DIFaddbegin \DIFadd{)\mbox{%DIFAUXCMD
\cite{grattafiori2024llama3} }\hskip0pt%DIFAUXCMD
/ Together AI / multi-GPU cloud / OpenAI-compatible API), pairing with L-LLaMA3 to isolate cloud serving from production-infrastructure complexity. Both L-LLaMA3 and C-LLaMA3 use the same Q4\_0 INT4 quantisation of the LLaMA~3 8B weights; the only difference is the serving infrastructure}\DIFaddend .
...
\DIFaddbegin \DIFadd{It is useful to distinguish two classes of factor that are often conflated in discussions of API reproducibility.
}\emph{\DIFadd{Cloud deployment factors}} \DIFadd{are those intrinsic to running a model on hardware accessed over a network: round-trip latency, request routing across geographically distributed endpoints, shared physical hardware, and multi-tenancy isolation.
}\emph{\DIFadd{Production serving infrastructure factors}} \DIFadd{are specific design choices made by LLM-serving stacks to maximise throughput and latency efficiency at scale: tensor parallelism, FlashAttention kernel non-determinism, dynamic batching with continuous request scheduling, speculative decoding fast paths, mixed-precision accumulation, and token-level prefix caching across requests.
...
\DIFaddend Six well-documented mechanisms \DIFaddbegin \DIFadd{in the production serving class }\DIFaddend can independently produce non-deterministic outputs under greedy decoding in distributed GPU inference:
...
\DIFaddbegin \textbf{\DIFadd{Mechanism activation per stack.}}
\DIFadd{Table~\ref{tab:mechanisms} summarises which of these six mechanisms are likely active for each evaluated stack.
For closed-source providers, mechanism activation cannot be verified directly and is necessarily inferred from public documentation, latency profiles, and standard production practice; we mark these entries as }\emph{\DIFadd{likely}} \DIFadd{or }\emph{\DIFadd{possible}} \DIFadd{accordingly.
}

\begin{table*}[h]
\caption{\textbf{\DIFaddFL{Mapping of non-determinism mechanisms to deployment stacks.}}
\DIFaddFL{Symbols: }\checkmark\DIFaddFL{, present and verifiable; ``likely'', consistent with public documentation and standard production practice; ``possible'', plausible but not confirmed; ---, absent or strongly mitigated; $\triangle$, present in principle but mitigated by design (single-GPU integer arithmetic).
For closed-source stacks, attribution is necessarily inferential.}}\label{tab:mechanisms}
\footnotesize
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{\DIFaddFL{Mechanism}} & \textbf{\DIFaddFL{L-LLaMA3}} & \textbf{\DIFaddFL{C-LLaMA3 (Together)}} & \textbf{\DIFaddFL{API-GPT4}} & \textbf{\DIFaddFL{API-Claude}} & \textbf{\DIFaddFL{API-Gemini}} \\
\midrule
\DIFaddFL{Non-associative FP arithmetic     }& \DIFaddFL{$\triangle$ }& \DIFaddFL{$\triangle$ }& \DIFaddFL{$\triangle$ }& \DIFaddFL{$\triangle$ }& \DIFaddFL{$\triangle$ }\\
\DIFaddFL{Mixed-precision BF16/FP16         }& \DIFaddFL{--- (Q4 int)}& \checkmark  & \DIFaddFL{likely      }& \DIFaddFL{likely      }& \DIFaddFL{likely }\\
\DIFaddFL{Tensor parallelism                }& \DIFaddFL{---         }& \checkmark  & \DIFaddFL{likely      }& \DIFaddFL{likely      }& \DIFaddFL{likely }\\
\DIFaddFL{FlashAttention kernel             }& \DIFaddFL{---         }& \DIFaddFL{likely      }& \DIFaddFL{likely      }& \DIFaddFL{likely      }& \DIFaddFL{likely }\\
\DIFaddFL{Dynamic batching                  }& \DIFaddFL{---         }& \checkmark  & \DIFaddFL{likely      }& \DIFaddFL{likely      }& \DIFaddFL{likely }\\
\DIFaddFL{Speculative decoding              }& \DIFaddFL{---         }& \DIFaddFL{possible    }& \DIFaddFL{likely      }& \DIFaddFL{likely      }& \DIFaddFL{likely }\\
\bottomrule
\end{tabular}
\end{table*}


\DIFaddend \subsection{Protocol overhead}\label{subsec:overhead}
...
(S10)~\DIFdelbegin \DIFdel{Experimental coverage matrix (model--task--condition run counts)}\DIFdelend \DIFaddbegin \DIFadd{Environment and provenance transparency documentation;
...
\DIFaddbegin \DIFadd{H.V.S.\ reviewed the manuscript and co-supervised the research.
}\DIFaddend E.T.B.\ contributed to experimental design and data analysis.
A.T.d.A.\ contributed to methodology design and statistical analysis.
\DIFdelbegin \DIFdel{H.V.S.\ contributed to research design and manuscript review.
}\DIFdelend Y.d.S.T.\ supervised the research, contributed to methodology design, and reviewed the manuscript.
...
\item \textbf{\DIFadd{Data availability:}}
\DIFadd{All experimental records (original 4}{\DIFadd{,}}\DIFadd{104 runs plus revision additions for HumanEval, GSM8K, the multi-turn extension, and the cross-domain PubMed probe), W3C PROV-JSON provenance documents, Run Cards, Prompt Cards, input data with DOIs, and source data for every figure and table are available to reviewers during peer review via a private Figshare share URL provided in the Cover Letter, and at the project's GitHub repository (}\url{https://github.com/Roverlucas/genai-reproducibility-protocol}\DIFadd{, release tag }\texttt{\DIFadd{v1.1-natcomms-revision1}}\DIFadd{). The Figshare deposit will be made publicly accessible at acceptance under a CC-BY~4.0 licence; its persistent DOI is }\href{https://doi.org/10.6084/m9.figshare.31653373}{\DIFadd{10.6084/m9.figshare.31653373}}\DIFadd{.
}

\item \textbf{\DIFadd{Code availability:}}
\DIFadd{The full source code for the provenance protocol (Run Card, Prompt Card, W3C PROV-JSON generation), the experiment runners (}\texttt{\DIFadd{run\_experiments.py}}\DIFadd{, }\texttt{\DIFadd{run\_claude\_multiturn.py}}\DIFadd{, }\texttt{\DIFadd{run\_gemini\_multiturn.py}}\DIFadd{, }\texttt{\DIFadd{run\_chat\_control.py}}\DIFadd{, and the resumable revision orchestrator }\texttt{\DIFadd{run\_revision\_full.sh}}\DIFadd{), the statistical and figure-generation pipeline, and the test suite (102 }\texttt{\DIFadd{pytest}} \DIFadd{tests) are available to reviewers during peer review at }\url{https://github.com/Roverlucas/genai-reproducibility-protocol} \DIFadd{(release tag }\texttt{\DIFadd{v1.1-natcomms-revision1}}\DIFadd{). On acceptance, the repository will be made publicly available under the MIT licence. A permanent mirror of the code archive is deposited on Figshare alongside the data with persistent DOI }\href{https://doi.org/10.6084/m9.figshare.31653373}{\DIFadd{10.6084/m9.figshare.31653373}} \DIFadd{(CC-BY 4.0). A }\texttt{\DIFadd{README.md}} \DIFadd{with installation and execution instructions and a pinned }\texttt{\DIFadd{requirements.txt}} \DIFadd{(Python~3.14.3) are provided.
...
\setcounter{table}{7}%DIF >  Align table number with Extended Data Table 8 stub above
\begin{table*}[h]
\centering
\footnotesize
...
\DIFdelbegin \bibitem{noy2023experimental}
\DIFdel{Noy, S. \& Zhang, W.
Experimental evidence on the productivity effects of generative artificial intelligence.
}\DIFdelend \DIFaddbegin \bibitem{birhane2023science}
\DIFadd{Birhane, A. }\emph{\DIFadd{et al.}}
\DIFadd{Science in the age of large language models.
}\DIFaddend \emph{\DIFdelbegin \DIFdel{Science}\DIFdelend \DIFaddbegin \DIFadd{Nat. Rev. Phys.}\DIFaddend } \textbf{\DIFdelbegin \DIFdel{381}\DIFdelend \DIFaddbegin \DIFadd{5}\DIFaddend }, \DIFdelbegin \DIFdel{187--192 }\DIFdelend \DIFaddbegin \DIFadd{277--280 }\DIFaddend (2023).
...
Preprint at }\url{https://arxiv.org/abs/2107.03374} \DIFadd{(2021).
}

\bibitem{cobbe2021gsm8k}
\DIFadd{Cobbe, K. }\emph{\DIFadd{et al.}}
\DIFadd{Training verifiers to solve math word problems.
Preprint at }\url{https://arxiv.org/abs/2110.14168} \DIFadd{(2021).
}
