...
%DIF DEL submission_revision_v1/ncomms_main_post_T5.tex   Fri May  8 16:40:42 2026
%DIF ADD article/ncomms_main.tex                          Sun May 10 21:46:01 2026
...
We conducted 4,104 controlled experiments \DIFdelbegin \DIFdel{across eight models and }\DIFdelend \DIFaddbegin \DIFadd{in the original analysis and an additional 2,900 experiments in revision (7,004 in total) across nine deployment stacks, eight models, }\DIFaddend five independent API providers\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, and six task families spanning structured extraction, scientific summarisation, multi-turn refinement, retrieval-augmented generation, code generation (HumanEval), and mathematical reasoning (GSM8K).
We }\DIFaddend found that even under temperature~$=$~0 with fixed random seeds, API-served models reproduce their own outputs only 22.1\% of the time \DIFaddbegin \DIFadd{on the original two tasks}\DIFaddend , while locally deployed open-weight models achieve 95.6\% exact \DIFdelbegin \DIFdel{reproducibility, a }\DIFdelend \DIFaddbegin \DIFadd{reproducibility---a }\DIFaddend gap exceeding four-fold (95\% bootstrap CI for the ratio: 2.48--3.61$\times$) \DIFaddbegin \DIFadd{that the revision experiments confirm extends to code, math, and out-of-AI/ML domains}\DIFaddend .
...
Fourth, we provide a lightweight provenance protocol grounded in \DIFaddbegin \DIFadd{the }\DIFaddend W3C \DIFdelbegin \DIFdel{PROV\mbox{%DIFAUXCMD
...
) under our standard C1 greedy decoding (temperature~$=$~0, fixed seed where supported) with five repetitions per problem across all eight deployment stacks with full coverage (Extended Data Table~\ref{tab:revision_emr}; full per-stack per-task EMR with 95\% bootstrap CIs in Supplementary Section~S11).
}
...
\DIFadd{The pattern observed on AI/ML abstracts replicates with no qualitative change (Extended Data Table~\ref{tab:revision_emr}).
The three local stacks (L-Gemma2, L-LLaMA3) and the Together AI quasi-isolation probe (C-LLaMA3) all achieve perfect EMR~$=$~1.000 }[\DIFadd{1.00,\,1.00}]\DIFadd{; L-Mistral attains 0.960 }[\DIFadd{0.88,\,1.00}]\DIFadd{.
...
\DIFadd{To probe whether the resulting textual divergence is cosmetic or substantive, we ran a blinded LLM-as-judge protocol on 10 newly sampled non-identical Claude output pairs from the PM\textsubscript{2.5} corpus---distinct from the 23 study-level cases analysed in the companion paper.
Each pair was scored by Claude Opus 4.7 against three pre-registered criteria (direction of the reported effect, magnitude within $\pm$20\%, and overlap of confidence intervals) under a blind comparison setup, with verdicts }\emph{\DIFadd{truly\_contradictory}}\DIFadd{, }\emph{\DIFadd{semantically\_equivalent}}\DIFadd{, or }\emph{\DIFadd{ambiguous}} \DIFadd{(cost: USD~0.28; full prompts in Supplementary Section~S12).
...
\begin{figure}[t]
  ...
  Heatmap of EMR \DIFdelbeginFL \DIFdelFL{for eight deployment stacks }\DIFdelendFL across extraction and summarization tasks under temperature~$=$~\DIFdelbeginFL \DIFdelFL{0.
...
\DIFaddbegin \DIFadd{The Together AI quasi-isolation result also has scope limits that we wish to make explicit.
...
\DIFaddend Indeed, preliminary evidence from an applied domain suggests that the effects reported here have direct consequences for scientific conclusions.
...
\DIFdelbegin \DIFdel{Four tasks }\DIFdelend \DIFaddbegin \DIFadd{Six task families }\DIFaddend spanning the output-structure spectrum\DIFaddbegin \DIFadd{.
The first four were evaluated in the original analysis}\DIFaddend :
...
Grand total \DIFaddbegin \DIFadd{for the original analysis}\DIFaddend : 4,104 logged runs across 9 deployments and 7 execution environments.
...
Revision total: 1}{\DIFadd{,}}\DIFadd{200 + 1}{\DIFadd{,}}\DIFadd{200 + 100 + 400 + 10 judge calls = 2}{\DIFadd{,}}\DIFadd{910 logged runs (rounded to 3}{\DIFadd{,}}\DIFadd{010 in the introduction together with the orchestration calls and provenance hash batches), bringing the manuscript-wide total to 7}{\DIFadd{,}}\DIFadd{114 logged generative-AI runs across 9 deployment stacks and 6 task families.
}
...
All \DIFdelbegin \DIFdel{4,104 }\DIFdelend run records (JSON), PROV-JSON provenance documents, Run Cards, Prompt Cards, input data (30 \DIFaddbegin \DIFadd{ML/AI }\DIFaddend abstracts with DOIs\DIFaddbegin \DIFadd{, 10 PubMed PM2.5 abstracts, HumanEval and GSM8K problem identifiers}\DIFaddend ), and \DIFdelbegin \DIFdel{generated figures }\DIFdelend \DIFaddbegin \DIFadd{source data underlying every figure and table }\DIFaddend are publicly available at \url{https://github.com/Roverlucas/genai-reproducibility-protocol} under \DIFaddbegin \DIFadd{a }\DIFaddend CC-BY\DIFaddbegin \DIFadd{~}\DIFaddend 4.0 \DIFdelbegin \DIFdel{license. }\DIFdelend \DIFaddbegin \DIFadd{licence. An archived snapshot of the release tag }\texttt{\DIFadd{v1.1-natcomms-revision1}} \DIFadd{will be deposited in Zenodo on acceptance and a persistent DOI will be inserted at proof stage. }\DIFaddend Source data are provided with this paper.

\bmhead{Code availability}

The reference implementation of the provenance protocol \DIFdelbegin \DIFdel{, all analysis scripts, }\DIFdelend \DIFaddbegin \DIFadd{(Run Card, Prompt Card, W3C PROV-JSON generation), all experiment runners, the statistical }\DIFaddend and figure-generation \DIFdelbegin \DIFdel{code }\DIFdelend \DIFaddbegin \DIFadd{pipeline, and the revision extensions for HumanEval, GSM8K and the cross-domain PubMed probe }\DIFaddend are publicly available at \url{https://github.com/Roverlucas/genai-reproducibility-protocol} under \DIFdelbegin \DIFdel{MIT License. }\DIFdelend \DIFaddbegin \DIFadd{the MIT licence. The repository includes a }\texttt{\DIFadd{README.md}} \DIFadd{with installation and execution instructions, pinned dependencies (}\texttt{\DIFadd{requirements.txt}}\DIFadd{, Python 3.14.3), and a test suite (}\texttt{\DIFadd{pytest}}\DIFadd{, 102 tests). An archived snapshot of release tag }\texttt{\DIFadd{v1.1-natcomms-revision1}} \DIFadd{will be deposited in Zenodo on acceptance, with the persistent DOI inserted at proof stage.
}\DIFaddend 
...
(S12)~LLM-as-judge protocol for the PubMed PM\textsubscript{2.5} probe: judge prompts, three pre-registered criteria (direction, magnitude $\pm$20\%, CI overlap), per-pair verdicts (5 truly\_contradictory / 3 semantically\_equivalent / 2 ambiguous), and cost log}\DIFaddend .
...
All \DIFaddbegin \DIFadd{experimental records (original }\DIFaddend 4\DIFdelbegin \DIFdel{,}\DIFdelend \DIFaddbegin {\DIFadd{,}}\DIFaddend 104 \DIFdelbegin \DIFdel{experimental records, provenance metadata, and input abstracts }\DIFdelend \DIFaddbegin \DIFadd{runs plus revision additions for HumanEval, GSM8K, the multi-turn extension, and the cross-domain PubMed probe), W3C PROV-JSON provenance documents, Run Cards, Prompt Cards, input data with DOIs, and source data for every figure and table }\DIFaddend are available to reviewers during peer review via the project repository at \url{https://github.com/Roverlucas/genai-reproducibility-protocol}. \DIFdelbegin \DIFdel{Upon publication}\DIFdelend \DIFaddbegin \DIFadd{On acceptance}\DIFaddend , an archived snapshot \DIFdelbegin \DIFdel{with a persistent DOI }\DIFdelend \DIFaddbegin \DIFadd{of release tag }\texttt{\DIFadd{v1.1-natcomms-revision1}} \DIFaddend will be deposited in Zenodo \DIFaddbegin \DIFadd{and a persistent DOI will be minted and inserted at proof stage. The repository will be made public }\DIFaddend under a CC-BY\DIFaddbegin \DIFadd{~}\DIFaddend 4.0 licence \DIFdelbegin \DIFdel{. Source data for all figures and tables are included in the repository.
}\DIFdelend \DIFaddbegin \DIFadd{on acceptance.
}\DIFaddend 

\item \textbf{Code availability:}
The full source code for the provenance protocol \DIFaddbegin \DIFadd{(Run Card, Prompt Card, W3C PROV-JSON generation), the experiment runners (}\texttt{\DIFadd{run\_experiments.py}}\DIFadd{, }\texttt{\DIFadd{run\_claude\_multiturn.py}}\DIFadd{, }\texttt{\DIFadd{run\_gemini\_multiturn.py}}\DIFaddend , \DIFdelbegin \DIFdel{experiment runners, analysis scripts, and figure generation is }\DIFdelend \DIFaddbegin \texttt{\DIFadd{run\_chat\_control.py}}\DIFadd{, and the resumable revision orchestrator }\texttt{\DIFadd{run\_revision\_full.sh}}\DIFadd{), the statistical and figure-generation pipeline, and the test suite (102 }\texttt{\DIFadd{pytest}} \DIFadd{tests) are }\DIFaddend available to reviewers during peer review at \url{https://github.com/Roverlucas/genai-reproducibility-protocol}. \DIFdelbegin \DIFdel{Upon publication}\DIFdelend \DIFaddbegin \DIFadd{On acceptance}\DIFaddend , the repository will be made publicly available under \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend MIT licence and an archived release with a persistent \DIFaddbegin \DIFadd{Zenodo }\DIFaddend DOI will be \DIFdelbegin \DIFdel{deposited in Zenodo. A README file }\DIFdelend \DIFaddbegin \DIFadd{minted and inserted into this statement at proof stage. A }\texttt{\DIFadd{README.md}} \DIFaddend with installation and execution instructions \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{and a pinned }\texttt{\DIFadd{requirements.txt}} \DIFadd{(Python~3.14.3) are }\DIFaddend provided.
\end{itemize}
...
\DIFaddFL{T5 = HumanEval (code); T6 = GSM8K (math); T3-ext = multi-turn refinement extended to gpt-4o and DeepSeek; T14 = 10 PubMed PM\textsubscript{2.5} abstracts (Task~2 extraction schema). 10\,000 percentile bootstrap resamples.}}
...
\emph{Res. Synth. Methods} (2026, submitted\DIFaddbegin \DIFadd{).
}
...
\DIFadd{Shi, F. }\emph{\DIFadd{et al.}}
\DIFadd{On the reliability of retrieval-augmented generation: a survey of failure modes and variability.
Preprint at }\url{https://arxiv.org/abs/2401.05856} \DIFadd{(2024}\DIFaddend ).
