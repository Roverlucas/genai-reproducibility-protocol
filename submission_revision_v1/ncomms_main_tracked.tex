...
%DIF ADD article/ncomms_main.tex                          Mon May 11 09:32:24 2026
...
We }\DIFaddend found that even under temperature~$=$~0 with fixed random seeds, API-served models reproduce their own outputs only 22.1\% of the time \DIFaddbegin \DIFadd{on the original two tasks}\DIFaddend , while locally deployed open-weight models achieve 95.6\% exact \DIFdelbegin \DIFdel{reproducibility, a gap exceeding four-fold }\DIFdelend \DIFaddbegin \DIFadd{reproducibility---a ratio of approximately $4\times$ in point estimates }\DIFaddend (95\% bootstrap CI\DIFdelbegin \DIFdel{for the ratio}\DIFdelend : 2.48--3.61$\times$)\DIFaddbegin \DIFadd{, with the revision experiments confirming the gap extends to code, math, and out-of-AI/ML domains}\DIFaddend .
...
\textbf{Normalized Edit Distance (NED):} Levenshtein distance\cite{levenshtein1966binary} \DIFdelbegin \DIFdel{normalized }\DIFdelend \DIFaddbegin \DIFadd{normalised }\DIFaddend by the longer string.
...
All \DIFdelbegin \DIFdel{4,104 }\DIFdelend run records (JSON), PROV-JSON provenance documents, Run Cards, Prompt Cards, input data (30 \DIFaddbegin \DIFadd{ML/AI }\DIFaddend abstracts with DOIs\DIFaddbegin \DIFadd{, 10 PubMed PM\textsubscript{2.5} abstracts, HumanEval and GSM8K problem identifiers}\DIFaddend ), and \DIFdelbegin \DIFdel{generated figures }\DIFdelend \DIFaddbegin \DIFadd{source data underlying every figure and table }\DIFaddend are publicly available at \url{https://github.com/Roverlucas/genai-reproducibility-protocol} under \DIFaddbegin \DIFadd{a }\DIFaddend CC-BY\DIFaddbegin \DIFadd{~}\DIFaddend 4.0 \DIFdelbegin \DIFdel{license. }\DIFdelend \DIFaddbegin \DIFadd{licence. An archived snapshot of the release tag }\texttt{\DIFadd{v1.1-natcomms-revision1}} \DIFadd{will be deposited in Zenodo on acceptance and a persistent DOI will be inserted at proof stage. }\DIFaddend Source data are provided with this paper.
