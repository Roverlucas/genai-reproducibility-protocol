...
\noindent\textbf{Re: Manuscript NCOMMS-26-021731A} (revised)
...
\begin{enumerate}[leftmargin=*,resume]
  \item \textbf{Coding (HumanEval) and math reasoning (GSM8K) tasks (Editor; R1.8; R3.1).} Per the editor's specific request, we ran new controlled experiments on 30 HumanEval problems and 30 GSM8K problems, evaluated under our standard greedy-decoding protocol across all stacks. We report exact-match reproducibility (EMR, with NED, ROUGE-L and BERTScore) under the same three-level framework---applied at the source-code level for HumanEval and to the full chain-of-thought plus final answer for GSM8K. The local--API reproducibility gap persists in both new domains, with patterns analogous to summarisation/extraction.
...
The companion paper has not yet been submitted elsewhere and is openly preregistered at OSF (\href{https://doi.org/10.17605/OSF.IO/VR934}{doi.org/10.17605/OSF.IO/VR934}). Given the natural complementarity between the two works---the present manuscript diagnoses the hidden non-determinism and provides a protocol to detect it, while the companion paper quantifies its applied impact on a 500-abstract environmental-health evidence base---we would welcome the opportunity to have both considered at \emph{Nature Communications}, should the editors find that of interest. The present manuscript, in any case, stands on its own.
