...
\abstract{Large language model APIs serve research in medicine, climate science, and engineering under settings documented as deterministic. We show this guarantee fails in practice. Across 7{,}004 experiments on nine \emph{deployment stacks} (weights, provider, infrastructure, API) and six task families (extraction, summarisation, multi-turn refinement, RAG, code, math), API-served stacks reproduce their own outputs as little as 1\% under temperature-zero greedy decoding with fixed seeds, while local stacks reach 92--100\%. The variation is invisible: outputs appear semantically equivalent (BERTScore F1~$>$~0.97) while differing textually, yet a blinded LLM-as-judge classifies half of the divergences as truly contradictory. The gap widens in high-stakes regimes (multi-turn, RAG, code, math). A quasi-isolation experiment with identical LLaMA~3~8B weights served by two stacks implicates production infrastructure rather than cloud deployment. We provide a W3C PROV-based provenance protocol ($<$1\% overhead) that makes this hidden variation detectable and auditable.}
...
We conducted 4,104 controlled experiments in the original analysis and an additional 2,900 experiments in revision (7,004 in total) across nine deployment stacks, eight models, five independent API providers, and six task families spanning structured extraction, scientific summarisation, multi-turn refinement, retrieval-augmented generation, code generation (HumanEval), and mathematical reasoning (GSM8K).
We found that even under temperature~$=$~0 with fixed random seeds, API-served models reproduce their own outputs only 22.1\% of the time on the original two tasks, while locally deployed open-weight models achieve 95.6\% exact reproducibility---a ratio of approximately $4\times$ in point estimates (95\% bootstrap CI: 2.48--3.61$\times$), with the revision experiments confirming the gap extends to code, math, and out-of-AI/ML domains.
...
To probe whether the resulting textual divergence is cosmetic or substantive, we ran a blinded LLM-as-judge protocol on 10 newly sampled non-identical output pairs from the PM\textsubscript{2.5} corpus---6 Claude Sonnet 4.5 cases and 4 GPT-4.1 cases, stratified by disagreement kind (direction, magnitude, CI overlap, mixed) and distinct from the 23 study-level cases analysed in the companion paper.
Each pair was scored by Claude Opus 4.7 against three pre-registered criteria (direction of the reported effect, magnitude within $\pm$20\%, and overlap of confidence intervals) under a blind comparison setup, with verdicts \emph{truly\_contradictory}, \emph{semantically\_equivalent}, or \emph{ambiguous} (cost: USD~0.28; full prompts in Supplementary Section~S12).
The judge classified 5 of the 10 pairs as truly contradictory (Wilson 95\% CI 24--76\%), 3 as semantically equivalent (11--60\%), and 2 as ambiguous (6--51\%); the sample is small (n=10), so the proportion estimates carry wide intervals, but the lower bound on the truly-contradictory share excludes a pure-cosmetic interpretation. Representative side-by-side examples of substantive (versus cosmetic) divergence for this and the structured-extraction task appear in Box~1.
Half of the divergences in the same Claude outputs that show BERTScore F1~$>$~0.97 are therefore substantive disagreements about the underlying scientific claim, not surface-level rephrasings: BERTScore alone cannot tell a cosmetic variation from a reversed direction of effect or a confidence interval that no longer overlaps the original.
...
A blinded LLM-as-judge analysis on 10 sampled non-identical Claude pairs from Task~T14 was run with Claude Opus 4.7 as the judge under three pre-registered criteria (direction, magnitude $\pm$20\%, CI overlap); full prompts and per-pair verdicts are in Supplementary Section~S12.
Revision total: 1{,}200 (HumanEval) + 1{,}200 (GSM8K) + 400 (PubMed PM\textsubscript{2.5}) + 100 (multi-turn extension) = 2{,}900 logged generative-AI runs, bringing the manuscript-wide total to 7{,}004 runs across 9 deployment stacks and 6 task families. An additional 10 LLM-as-judge calls (Claude Opus 4.7, see Section~\ref{sec:s12}) support the applied-impact triangulation but are reported separately as they belong to the validation layer rather than the primary experimental matrix.
...
\textbf{Normalized Edit Distance (NED):} Levenshtein distance\cite{levenshtein1966binary} normalised by the longer string.
...
(S12)~LLM-as-judge protocol for the PubMed PM\textsubscript{2.5} probe: judge prompts, three pre-registered criteria (direction, magnitude $\pm$20\%, CI overlap), per-pair verdicts (5 truly\_contradictory / 3 semantically\_equivalent / 2 ambiguous), and cost log.
...
T5 = HumanEval (code); T6 = GSM8K (math); T3-ext = multi-turn refinement extended to gpt-4o and DeepSeek; T14 = 10 PubMed PM\textsubscript{2.5} abstracts (Task~2 extraction schema). 10\,000 percentile bootstrap resamples.}
...
\bibitem{shi2024ragvariability}
Shi, F. \emph{et al.}
On the reliability of retrieval-augmented generation: a survey of failure modes and variability.
Preprint at \url{https://arxiv.org/abs/2401.05856} (2024).
