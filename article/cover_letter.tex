...
\begin{enumerate}
    ...
    \item \textbf{An empirical evaluation} through 1,864 controlled experiments with LLaMA~3 8B (local) and GPT-4 (API) across 30 scientific abstracts, documenting a striking reproducibility gap between local and API-based inference. Notably, GPT-4 achieves only 23\% exact match rate for summarization under nominally deterministic settings---a finding that is invisible without systematic logging.
...
A supplementary chat-format control experiment (200 additional runs using Ollama's \texttt{/api/chat} endpoint) confirms that the prompt-format difference between the two deployment paradigms does not explain the reproducibility gap, strengthening the internal validity of our findings.
