...
\caption{Visual abstract summarizing the study design and key findings. \textbf{Left:} The provenance protocol pipeline, from Prompt Card creation through Run Card logging and W3C PROV graph generation. \textbf{Center:} Exact Match Rates (EMR) under greedy decoding for all models, illustrating the reproducibility gap between local models (green, EMR $\geq$ 0.960) and API-served models (orange/red, EMR $\leq$ 0.800). \textbf{Right:} Key statistics from the study: 3,904 experiments, 8 models, 4 tasks, 5 API providers, with less than 1\% protocol overhead.}
...
\textbf{Finding 3: All four API-served models exhibit non-determinism under greedy decoding, observed independently across four providers.} Under $t{=}0$, DeepSeek Chat achieves the highest API reproducibility (EMR = 0.800 for extraction), followed by GPT-4 (0.443/0.230), Claude Sonnet 4.5 (0.190/0.020), and Perplexity Sonar (0.100/0.010). Claude's EMR of 0.020 for summarization means that across 10~abstracts $\times$ 5 repetitions, effectively no two runs produce the same output.
...
\textbf{API non-determinism is observed across all four providers.} Our most consequential finding is that \textit{all four} API-served models---GPT-4 (OpenAI), Claude Sonnet 4.5 (Anthropic), DeepSeek Chat (DeepSeek), and Perplexity Sonar (Perplexity)---exhibit non-determinism under greedy decoding on single-turn tasks, albeit at substantially different magnitudes (EMR ranging from 0.800 for DeepSeek Chat down to 0.010 for Perplexity Sonar). Researchers using \textit{any} API-served model should never assume reproducibility without verification and should report multiple runs with variability metrics.
...
The difference between summarization and extraction reproducibility---observed consistently across all seven models---is consistent with and extends our earlier two-model finding. The reproducibility hierarchy (extraction $>$ summarization) holds for local models (EMR gap of 0.03--0.12) and for API models (EMR gap of 0.04--0.21, with the narrowest gap for DeepSeek Chat and the widest for GPT-4). This finding suggests a spectrum ranging from highly constrained tasks (structured extraction) to open-ended tasks (summarization), with the degree of output-space constraint serving as a primary determinant.
