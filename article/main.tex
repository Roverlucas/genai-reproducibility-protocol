...
Second, field-level exact match rates reveal that structured fields (\texttt{benchmark}, \texttt{model\_or\_system}) are 2--6$\times$ more reproducible than open-ended fields (\texttt{method}, \texttt{key\_result}), consistent with the finding that structured tasks achieve higher overall EMR. The absolute values of field-level EMR are notably low (e.g., 0.071 overall for LLaMA~3 at $t{=}0$) compared with the overall extraction EMR of 0.987 reported in Table~\ref{tab:variability-results}. This discrepancy reflects three factors: (i)~\textit{granularity}: field-level EMR compares short strings (typically 2--15 words) where a single word substitution or reordering causes a mismatch, whereas overall EMR compares complete outputs where the same minor variation is diluted; (ii)~\textit{cross-abstract pooling}: field-level EMR is computed across all pairwise combinations of runs within a condition (including runs from different abstracts), so most pairs compare inherently different content---the metric thus captures between-abstract variation in addition to within-abstract reproducibility; (iii)~\textit{open-ended fields}: fields like \texttt{method} and \texttt{key\_result} admit many valid phrasings for the same underlying fact, depressing exact match rates even when the extracted information is semantically correct. Researchers should therefore interpret field-level EMR as a lower bound on reproducibility at the finest granularity, not as a contradiction of the high overall EMR values.
