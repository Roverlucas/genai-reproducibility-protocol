...
Figure~\ref{fig:visual-abstract} provides an overview of the study design, protocol pipeline, and key findings. In this paper, we make three contributions, with the protocol design as the primary and most durable contribution:
...
\begin{enumerate}
    ...
    \item \textbf{A large-scale empirical case study} demonstrating both the protocol's effectiveness and the reproducibility characteristics of LLM outputs in the models and snapshots evaluated. Through 4,104 controlled experiments with nine model deployments (3~local, 5~closed-source API, 1~cloud-served open-weight for quasi-isolation probe) across four NLP tasks, 30 abstracts, seven execution environments (one local plus six cloud API providers), and five conditions (Section~\ref{sec:experimental-setup}), we document a reproducibility gap between local and API-based inference that is invisible without systematic logging.
