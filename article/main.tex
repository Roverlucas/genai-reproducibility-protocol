...
Table~\ref{tab:emr_greedy} presents the headline result: Exact Match Rates under greedy decoding for all eight models. Table~\ref{tab:three_level} provides the full three-level reproducibility assessment.
...
\begin{figure}[t]
    ...
    \caption{Heatmap of Exact Match Rate under greedy decoding for seven models. The horizontal white line separates local models (top three, green) from API-served models (bottom four, red/orange). Gemma~2 9B achieves perfect 1.000 across all tasks; DeepSeek Chat achieves the highest API reproducibility.}
    \Description{A heatmap with 7 rows (models) and 2 columns (tasks), using a red-yellow-green colormap where green indicates high EMR and red indicates low EMR.}
    \label{fig:heatmap}
\end{figure}
...
\begin{figure}[t]
    ...
    \Description{Radar chart with five axes showing EMR extraction, EMR summarization, 1-NED, ROUGE-L, and BERTScore for seven models (three local, four API).}
    \label{fig:radar}
\end{figure}
...
Table~\ref{tab:overhead} presents the protocol's overhead metrics across all eight models.

\input{tables/table_overhead.tex}

The protocol adds less than 1\% overhead for all eight models, with mean logging time ranging from 21--30\,ms depending on the model and task. Storage overhead remains modest at approximately 4\,KB per run record. The overhead is consistent across local and API deployment modes, indicating that the protocol is deployment-agnostic.
...
\begin{itemize}
    ...
    \item \textbf{Runtime cost}: $<$30\,ms per run across all eight models, negligible compared to inference times of seconds to minutes for typical LLM calls.
...
\begin{enumerate}
    ...
    \item \textbf{Comprehensive provenance logging adds negligible overhead}: less than 1\% of inference time and approximately 4\,KB per run across all eight models, removing any practical argument against systematic documentation.
\end{enumerate}
