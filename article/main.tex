...
We first define our key construct. \textbf{Provider-level non-determinism} is operationally defined as EMR~$<$~1.0 across identical API calls with fixed seed, temperature$\,{=}\,$0, and prompt. This definition makes no claim about the \textit{mechanism} producing the variability; it captures only the \textit{observable} phenomenon of non-identical outputs under nominally identical inputs.
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
    ...
    \item[\ding{51}] \textbf{Hyperparameters}: All inference parameters documented per model (Appendix~\ref{app:payloads}); temperature = 0, max\_tokens = 1024 across all models.
    ...
    \item[\ding{51}] \textbf{Computing infrastructure}: Hardware and software environment documented (Apple M4, Ollama v0.15.5, Python 3.14).
    \item[\ding{51}] \textbf{Run time}: Protocol overhead measured at $<$1\% of inference time ($\approx$4~KB per run).
\end{itemize}
