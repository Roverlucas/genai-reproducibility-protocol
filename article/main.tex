...
Second, field-level exact match rates reveal that structured fields (\texttt{benchmark}, \texttt{model\_or\_system}) are 2--5$\times$ more reproducible than open-ended fields (\texttt{method}, \texttt{key\_result}), consistent with the finding that structured tasks achieve higher overall EMR.
