...
\begin{enumerate}
    ...
    \item \textbf{An empirical evaluation} of both the protocol's effectiveness and the reproducibility characteristics of LLM outputs. Through 1,864 controlled experiments with LLaMA~3 8B (local) and GPT-4 (API) across two tasks, 30 abstracts, and five conditions, we quantify output variability using four complementary metrics and measure the protocol's overhead. Our results reveal a striking reproducibility gap between local and API-based inference that is invisible without systematic logging.
...
Second, field-level exact match rates reveal that structured fields (\texttt{benchmark}, \texttt{model\_or\_system}) are 2--6$\times$ more reproducible than open-ended fields (\texttt{method}, \texttt{key\_result}), consistent with the finding that structured tasks achieve higher overall EMR. Note that field-level EMR measures pairwise exact match on individual extracted field values (short strings sensitive to minor wording differences), whereas the overall extraction EMR in Table~\ref{tab:variability-results} measures whole-output character identity; these are complementary but distinct metrics.
