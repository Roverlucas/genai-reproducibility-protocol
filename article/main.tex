...
Each condition is applied to 30~abstracts $\times$ 2~tasks = 60~groups per condition.
Total: 1,864 logged runs (1,140 LLaMA~3 + 724 GPT-4).
\end{table}
...
Storage overhead is similarly modest: each run record occupies approximately 4.15\,KB, and the complete set of 1,864 run logs, provenance documents, and Run Cards totals 19.52\,MB.
...
\begin{enumerate}
    ...
    \item \textbf{Comprehensive provenance logging adds negligible overhead}: 0.545\% of inference time and 4.15\,KB per run, thereby removing any practical argument against systematic documentation.
\end{enumerate}
