...
To demonstrate that PROV-based reasoning goes beyond what plain JSON logs provide, we implemented three programmatic queries over our 4,104-run PROV dataset:

\begin{enumerate}
    \item \textbf{Divergence attribution}: ``For all abstract--condition groups with non-identical outputs, identify which PROV entities diverge.'' Result: across divergent groups for all five API models (GPT-4, Claude, Gemini, DeepSeek, Perplexity), 100\% share identical Prompt, InputText, ModelVersion, and InferenceParameters entities---the \textit{only} varying component is the RunGeneration activity, providing systematic evidence for server-side non-determinism across the entire dataset rather than anecdotal examples.

    \item \textbf{Cross-provider comparison}: ``Find all abstract--task pairs where multiple API models were given identical Prompt and InputText entities (verified by matching \texttt{genai:hash} attributes) but produced different Output entities.'' Result: across 10~abstracts $\times$ 2~tasks, all five API providers produced non-identical outputs across repetitions on shared inputs, consistent with provider-independent non-determinism---though with varying severity (DeepSeek Chat showing the least variability).
...
Our experiments establish \textit{that} API-served models exhibit non-determinism under greedy decoding, while local single-GPU models do not. Here we discuss the technical mechanisms that plausibly explain \textit{why}, drawing on the systems and numerical-analysis literature. While we cannot observe the internal architecture of closed API services, six well-documented mechanisms in distributed GPU inference can independently produce non-deterministic outputs even under greedy decoding---and all six are standard components of cloud LLM serving stacks.

\textbf{(1) Non-associative floating-point arithmetic.}
The fundamental root cause is that floating-point addition is non-associative: $(a + b) + c \neq a + (b + c)$ in finite precision \cite{higham2002accuracy}. Every mechanism below ultimately reduces to this property---different execution orders produce different numerical results, which can flip the argmax at the decoding step and cascade through the entire generation.

\textbf{(2) Mixed-precision accumulation.}
Modern LLM inference uses reduced-precision formats (FP16 or BF16) with only 10 or 7 bits of mantissa, respectively \cite{micikevicius2018mixed}. \citet{yuan2025nondeterminism} demonstrated that BF16 is ``the primary culprit'' for inference non-determinism: under greedy decoding with BF16, a 7B-parameter model showed up to 9\% accuracy variation and 9,000-token output-length differences across different GPU configurations. Their LayerCast mitigation---storing weights in BF16 but upcasting to FP32 for matrix multiplications---substantially reduces but does not eliminate non-determinism, and is unavailable to API consumers.

\textbf{(3) Tensor parallelism and all-reduce non-determinism.}
Cloud-served LLMs are typically distributed across multiple GPUs via tensor parallelism \cite{shoeybi2019megatron}, which splits matrix multiplications across devices and combines partial results via all-reduce collective operations. Because all-reduce aggregates partial sums from multiple GPUs, the order of accumulation depends on network timing and GPU synchronization---and given non-associative floating-point arithmetic, different accumulation orders produce different results. A single-GPU local deployment eliminates this source entirely: all operations execute on one device in a fixed order.

\textbf{(4) Attention kernel non-determinism.}
FlashAttention \cite{dao2022flashattention}, now the standard attention implementation in production LLM serving, introduces non-determinism through its parallelization strategy. \citet{golden2024flashstable} showed that FlashAttention produces roughly an order of magnitude more numerical deviation than baseline attention at BF16 precision, because its tiling strategy accumulates partial softmax results across thread blocks in implementation-dependent order. While the forward pass can be made deterministic with careful implementation, the non-deterministic numerical noise from attention computation compounds across layers.

\textbf{(5) Dynamic batching and request scheduling.}
Production LLM serving systems use continuous batching \cite{yu2022orca, kwon2023vllm}, where new requests are inserted into running batches at each decoding step. Different batch compositions lead to different memory access patterns, different padding configurations, and---importantly---different floating-point accumulation patterns in batched matrix multiplications. Since batch composition depends on concurrent request arrivals (which vary across runs), two identical requests processed at different times will be batched with different neighbors, potentially producing different outputs.

\textbf{(6) Speculative decoding.}
Many production LLM deployments use speculative decoding \cite{leviathan2023speculative} to reduce latency, in which a smaller draft model proposes multiple tokens that the target model then verifies in parallel. The acceptance/rejection sampling step introduces an additional stochastic component: if the draft model's confidence varies (e.g., due to batching effects on the draft model itself), the set of accepted tokens can differ across runs even with the same seed.

\textbf{Why local models escape these mechanisms.}
Our local deployment (single Apple M4 GPU, Ollama server, one request at a time) eliminates mechanisms (3)--(6) entirely: there is no tensor parallelism, no dynamic batching, no speculative decoding, and the attention kernel executes on a single device with deterministic thread scheduling. Mechanism (2) is mitigated by the specific quantization format used by Ollama (GGML Q4), which uses integer arithmetic for the core computation. The near-perfect reproducibility of our local models (average EMR = 0.960) is thus a \textit{predicted consequence} of the absence of these distributed-systems mechanisms, not merely an empirical observation.

\textbf{Quasi-isolation probe: empirical evidence.}
These six mechanisms share a critical property: they are all \textit{infrastructure-level} factors invisible to the API consumer. Our quasi-isolation probe experiment provides direct evidence for this claim: LLaMA~3 8B served via Together AI's cloud endpoint achieves EMR = 1.000 for extraction and 0.880 for summarization---nearly identical to the locally deployed version (1.000 and 0.920)---while major closed-source API models exhibit EMR $\leq$ 0.443 on the same tasks. While we lack visibility into Together AI's exact infrastructure, the Lite endpoint's INT4 quantization and competitive latency suggest a simpler serving stack than the multi-GPU clusters used for GPT-4, Claude, and Gemini. This result is consistent with mechanisms (3)--(6) being the primary drivers of non-determinism in production API services, though we cannot rule out alternative explanations without full infrastructure transparency. A researcher specifying \texttt{temperature=0} and \texttt{seed=42} has no control over the GPU cluster's tensor-parallelism strategy, the serving system's batching policy, or the precision format used for matrix multiplications. This asymmetry between user-visible parameters and infrastructure-level factors explains why greedy decoding does not guarantee determinism for API-served models, and motivates our protocol's emphasis on logging infrastructure metadata alongside generation parameters.
...
\begin{itemize}
    ...
    \item \textbf{Runtime cost}: $<$30\,ms per run across all models tested (Table~\ref{tab:overhead}), negligible compared to inference times of seconds to minutes for typical LLM calls.
    \item \textbf{Storage cost}: $\sim$4\,KB per run. Our 4,104 runs total approximately 16\,MB---less than a single model checkpoint.
...
Claude Sonnet 4.5 & Extraction & 10 (49)$^\ddagger$ & 10 (50) & 10 (30) & 10 (30) & 10 (30) \\
 ...
 & RAG & 10 (50) & -- & -- & -- & -- \\
\addlinespace
Gemini 2.5 Pro & Multi-turn & 10 (50) & -- & -- & -- & -- \\
 & RAG & 10 (50) & -- & -- & -- & -- \\
\addlinespace
DeepSeek Chat & Extraction & 10 (50) & -- & -- & -- & -- \\
 & Summarization & 10 (50) & -- & -- & -- & -- \\
\addlinespace
Perplexity Sonar & Extraction & 10 (50) & -- & -- & -- & -- \\
 & Summarization & 10 (50) & -- & -- & -- & -- \\
\addlinespace
Together AI LLaMA 3 8B & Extraction & 10 (50) & 10 (50) & -- & -- & -- \\
 & Summarization & 10 (50) & 10 (50) & -- & -- & -- \\
\bottomrule
\end{tabular}
\par\smallskip\raggedright\footnotesize
...
$^\S$An additional 200 runs (LLaMA~3 8B, chat-format control; Appendix~\ref{app:chat-control}) are not shown in this table.
\end{table*}
...
\section{Environment and Provenance Transparency}
\label{app:environment}

This appendix documents the complete execution environment and addresses the code-versioning gap identified in Section~\ref{sec:limitations}.

\subsection*{Execution Environment}

All local experiments were conducted on a single machine with the following configuration:
...
Network & Residential broadband (API calls only) \\
\bottomrule
\end{tabular}
\end{table}

Each of the 4,104 run records captures the following environment fields, regardless of git status: \texttt{os}, \texttt{os\_version}, \texttt{os\_release}, \texttt{architecture}, \texttt{processor}, \texttt{python\_version}, \texttt{hostname}, and \texttt{timestamp}. These fields are hashed into an \texttt{environment\_hash} (SHA-256) that serves as an immutable fingerprint. All local runs share the same environment hash (\texttt{cd45d428...}), confirming that no system changes occurred during the experiment window.

\subsection*{Code Versioning Timeline}

Table~\ref{tab:code-versioning} documents the relationship between experiment execution and repository commits.

\begin{table}[H]
\centering
\caption{Code versioning timeline: experiment phases and git status.}
\label{tab:code-versioning}
\small
\begin{tabular}{@{}lllr@{}}
\toprule
\textbf{Date} & \textbf{Phase} & \textbf{\texttt{code\_commit}} & \textbf{Runs} \\
\midrule
Feb 7, 2026 & Initial experiments & \texttt{no-git-repo} & 330 \\
Feb 7, 2026 & Repository initialized & \texttt{ff025ef} & --- \\
Feb 8--9, 2026 & Expanded experiments & \texttt{957a604}--\texttt{0cba4f1} & 1,874 \\
Feb 9, 2026 & Claude experiments & \texttt{e57ae19}--\texttt{df2b07e} & 1,500 \\
Feb 10, 2026 & Gemini experiments & \texttt{b2e9440} & 200 \\
Feb 9, 2026 & Chat-format control & \texttt{0cba4f1} & 200 \\
\midrule
\multicolumn{3}{@{}l}{\textit{Total with valid \texttt{code\_commit}}} & 3,774 (92\%) \\
\multicolumn{3}{@{}l}{\textit{Total without \texttt{code\_commit} (pre-git)}} & 330 (8\%) \\
\bottomrule
\end{tabular}
\end{table}
...
For the 330 runs executed before repository initialization, we verified provenance integrity through three independent checks:

\begin{enumerate}
    \item \textbf{Prompt hash consistency.} The \texttt{prompt\_hash} values in all 330 affected run records match the SHA-256 hashes of the prompt templates committed in the initial release (\texttt{ff025ef}). This confirms that the prompts used during experiments are identical to those in the repository.

    \item \textbf{Output hash reproducibility.} The analysis scripts (\texttt{compute\_metrics.py}, \texttt{merge\_all\_runs.py}) successfully process all 330 run records, producing consistent metrics. The output hashes in these records remain valid SHA-256 digests of the stored \texttt{output\_text} fields.

    \item \textbf{Environment hash stability.} All 330 pre-git runs share the same \texttt{environment\_hash} as the subsequent 1,874 runs on the same machine, confirming no system-level changes between the pre-git and post-git experiment phases.
\end{enumerate}

\subsection*{Representative Run Record}

Listing~\ref{lst:run-record} shows an abbreviated run record for a single LLaMA~3 8B extraction run. The complete set of 4,104 run records is available in the repository at \texttt{outputs/runs/}.

\begin{lstlisting}[caption={Abbreviated run record (LLaMA~3 8B, extraction, C1, rep\,0). Output text truncated for space.},label={lst:run-record}]
{
