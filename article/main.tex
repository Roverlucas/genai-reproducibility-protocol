...
\caption{Visual abstract summarizing the study design and key findings. \textbf{Left:} The provenance protocol pipeline, from Prompt Card creation through Run Card logging and W3C PROV graph generation. \textbf{Center:} Exact Match Rates (EMR) under greedy decoding for all seven models, illustrating the reproducibility gap between local models (green, EMR $\geq$ 0.960) and API-served models (orange/red, EMR $\leq$ 0.800). \textbf{Right:} Key statistics from the study: 3,804 experiments, 7 models, 4 tasks, 4 API providers, with less than 1\% protocol overhead.}
\Description{A three-panel visual abstract showing the protocol pipeline (left), a bar chart of EMR values for 7 models with a 3x local-vs-API gap annotation (center), and summary statistics (right).}
...
\textbf{DeepSeek Chat} (DeepSeek): Accessed via the OpenAI-compatible API. DeepSeek Chat represents a third independent API provider, allowing us to test whether the non-determinism pattern generalizes beyond the two largest Western providers. No seed parameter is supported; we set \texttt{temperature=0} for greedy decoding. DeepSeek Chat is evaluated under conditions C1 and C2 on single-turn tasks only (10 abstracts, 100 runs).

\textbf{Perplexity Sonar} (Perplexity): An online, search-augmented language model accessed via the Perplexity API. Unlike the other six models, Sonar incorporates real-time web search results into its generation context, introducing an additional source of variability beyond model-internal non-determinism. This model represents a fourth independent API provider and a qualitatively different deployment paradigm (search-augmented generation). Evaluated under C1 on single-turn tasks only (10 abstracts, 100 runs).
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
    ...
    \item[\ding{51}] \textbf{Data availability}: All 3,804 run records, PROV documents, and 30 input abstracts included in the repository.
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
    ...
    \item[\ding{51}] Automated test suite (49 tests) validates core analysis functions.
