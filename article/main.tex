...
\textbf{DVC} \cite{miao2023dvc} provides data versioning through git-like operations. While effective for dataset management, it does not address run-level provenance or prompt documentation.
...
\begin{enumerate}
    ...
    \item \textbf{Interoperability}: All artifacts are stored in open, machine-readable formats (JSON, PROV-JSON) to enable tool integration and long-term preservation.
\end{enumerate}
...
\input{../analysis/table2_main_results_v2.tex}
...
\input{../analysis/table_model_comparison.tex}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{../analysis/figures/fig_temperature_both_models.pdf}
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{../analysis/figures/fig_model_comparison.pdf}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{../analysis/figures/fig_heatmap_both_models.pdf}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{../analysis/figures/fig_overhead_both_models.pdf}
...
\textbf{Include warm-up runs for local models.} The per-abstract analysis revealed that the first inference call after model loading may differ from subsequent calls owing to cache initialization effects. Discarding the first run is a straightforward practice that improves measured reproducibility.
...
\begin{enumerate}
    ...
    \item \textbf{Compliance}: For regulated domains (healthcare, legal, finance), PROV documents provide the formal evidence trail required by audit standards.
\end{enumerate}
...
\textbf{Two tasks.} While summarization and extraction represent distinct points on the output-structure spectrum, they do not cover the full range of generative AI applications (e.g., dialogue, code generation, reasoning chains). A broader task suite would strengthen the generalizability of our findings.

\textbf{English-only, narrow domain.} Our input data consists of five English scientific abstracts, all drawn from high-profile NLP/ML papers. This selection may not be representative of scientific writing more broadly (e.g., biomedical, social science, or humanities abstracts), and the reproducibility characteristics we observe may differ for other languages, domains, or document types.

\textbf{No multi-turn evaluation.} All experiments use single-turn interactions. Multi-turn dialogues introduce additional variability through conversation history, which our current protocol logs but our experiments do not evaluate.

\textbf{Privacy considerations.} The protocol's environment metadata includes the machine hostname, which may reveal institutional information in sensitive contexts. Deployments in privacy-sensitive settings should consider anonymizing or omitting this field.

\textbf{Computational cost.} The total computational cost of our 330 experiments was modest: approximately 0.5 GPU-hours on a consumer laptop (Apple M4, 24\,GB) for 190 LLaMA~3 runs, plus 140 API calls to GPT-4. We estimate the carbon footprint to be negligible given this scale. Nevertheless, larger-scale adoption of the protocol across thousands of runs would not materially increase energy consumption, as the logging overhead (34\,ms per run) is dominated by the inference itself.
...
\begin{enumerate}
    \item \textbf{Local inference is substantially more reproducible than API-based inference.} Under identical greedy decoding settings, LLaMA~3 achieves EMR = 1.000 for extraction while GPT-4 achieves only 0.520, revealing significant server-side non-determinism that is invisible without systematic logging.
