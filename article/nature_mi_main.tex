...
We provide a lightweight provenance protocol ($<$1\% overhead) that makes this variation detectable, raising a reliability concern for LLMs in medicine, physical sciences, and automated data analysis.}
...
They encode clinical knowledge\cite{singhal2023large}, assist diagnostic workflows\cite{thirunavukarasu2023large}, accelerate literature review and data extraction\cite{noy2023experimental}, and are increasingly integrated into peer review at scale\cite{thakkar2026llm}.
...
Our experiments show that this guarantee does not hold in practice.
...
Averaging across all four tasks (including multi-turn and RAG; see Fig.~\ref{fig:multiturn}), the overall local average EMR is 0.956 versus 0.221 for API models, meaning that fewer than one in four API output pairs are identical under conditions documented as deterministic.
This gap survives Holm--Bonferroni correction across 68 hypothesis tests (51 significant at $\alpha$~$=$~0.05; Supplementary Section~S9) and is confirmed by large Cliff's delta effect sizes ($\delta$~$=$~0.784--0.896; Extended Data Table~2), paired $t$-test (Cohen's $d$~$>$~1.6), and a balanced 10-abstract subsample analysis that controls for sample-size differences between models (local EMR~$=$~0.953 vs.\ API EMR~$=$~0.304, 3.1$\times$ gap; Extended Data Table~4).
...
This three-level dissociation (low bitwise identity, moderate surface divergence, high semantic preservation) defines ``hidden'' non-determinism: same meaning, different words (Extended Data Table~1; Extended Data Fig.~1).
...
API models, however, start from an already-low baseline at $t$~$=$~0 and show a more complex pattern that violates the expected monotonicity (Extended Data Table~3).
...
\begin{figure}[t]
  ...
  Heatmap of EMR for model deployments across extraction and summarization tasks
  under temperature~$=$~0.
  ...
  Full results for all eight models appear in Table~\ref{tab:main}.
  }\label{fig:heatmap}
\end{figure}
...
\begin{tabular}{@{}llccc@{}}
\toprule
\textbf{Model} & \textbf{Deployment} & \textbf{$n$} & \textbf{Extraction EMR} & \textbf{Summarization EMR} \\
\midrule
...
The provenance protocol we provide offers a practical path forward (Fig.~\ref{fig:overview}).
...
Its key property is \emph{differential diagnosis}: when all hashes match except the output hash, the divergence is automatically attributed to the generation process, a result confirmed across all 4,104 runs (Supplementary Section~S4; Extended Data Fig.~2).
...
In summary, we have shown that the ``deterministic'' settings offered by major LLM API providers do not deliver determinism in practice, creating a hidden reproducibility gap that affects any study relying on API-served models.
...
(S10)~Experimental coverage matrix (model--task--condition run counts).
...
H.V.S.\ contributed to research design and manuscript review.
...
\bibitem{noy2023experimental}
Noy, S. \& Zhang, W.
Experimental evidence on the productivity effects of generative artificial intelligence.
\emph{Science} \textbf{381}, 187--192 (2023).
