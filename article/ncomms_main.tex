...
Across all models and conditions, whole-output BERTScore F1 remains above 0.97 even when EMR approaches zero (at the finer per-field level it remains above 0.94; Supplementary Section~S13).
...
HumanEval problem identifiers (30 stratified) for Task~5, GSM8K test-split identifiers (30 sampled) for Task~6, and 10 PubMed abstracts with DOIs for Task~T14 (full lists and DOIs in Supplementary Section~S2 (the 30 AI/ML abstracts) and Section~S11 (benchmark and PubMed identifiers)).
...
(S11)~Code, mathematics, and cross-domain task details: HumanEval problem identifiers, GSM8K problem identifiers, gpt-4o-2024-11-20 multi-turn extension protocol, and full per-stack per-task EMR with 95\% bootstrap CIs;
...
(S13)~Per-field reproducibility on the structured-extraction task (per-stack per-field EMR, NED, ROUGE-L, BERTScore).
