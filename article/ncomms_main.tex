...
\affil[2]{\orgname{Federal University of Paran\'a (UFPR)},
\orgaddress{\country{Brazil}}}

\affil[3]{\orgname{University of Campinas (Unicamp)},
\orgaddress{\country{Brazil}}}
...
We conducted 7{,}004 controlled experiments across nine deployment stacks, eight models (counting OpenAI's GPT-4 family---the gpt-4-0613 and gpt-4o-2024-11-20 snapshots---as a single model, and LLaMA~3 8B once despite being served on two stacks), five independent API providers, and six task families: structured extraction, scientific summarisation, multi-turn refinement, retrieval-augmented generation, code generation (HumanEval), and mathematical reasoning (GSM8K).
We found that even under temperature~$=$~0 with fixed random seeds, API-served models reproduce their own outputs only 22.1\% of the time on Tasks~1--2 (averaged across GPT-4 and Claude Sonnet 4.5, the two API stacks with full Tasks~1--4 coverage). Locally deployed open-weight models achieve 95.6\% exact reproducibility on the same tasks---a ratio of approximately $4\times$ in point estimates ($3.1\times$ under the balanced 10-abstract subsample, 95\% bootstrap CI: 2.48--3.61$\times$; Extended Data Table~4), and the gap extends to code, math, and out-of-AI/ML domains.
...
While individual reports have noted that ``deterministic'' API settings can fail\cite{atil2024nondeterminism,ouyang2024nondeterminism}, our first contribution is to quantify the phenomenon systematically: a controlled local-versus-API contrast at bitwise (SHA-256) resolution across all five major API providers tested (OpenAI, Anthropic, Google, DeepSeek, Perplexity) and six task families, establishing it as a general property of cloud-served LLM APIs rather than a provider- or task-specific anomaly.
...
Third, we demonstrate that cloud deployment \emph{per se} does not cause non-determinism: the same LLaMA~3 8B weights served via two stacks (local Ollama versus Together AI's cloud endpoint) achieve near-identical exact match rates. This is consistent with production-infrastructure complexity---tensor parallelism, speculative decoding, dynamic batching---rather than the cloud medium itself, and indicates that the deployment stack, not the model in isolation, is the carrier of variation.
...
Under the same greedy decoding, GPT-4 achieves EMR~$=$~0.443 [0.32,\,0.57] for extraction and 0.230 [0.16,\,0.30] for summarisation (variable-seed condition C2, as the OpenAI seed parameter is only advisory; the seed-free local and Claude stacks use C1; see Methods).
...
The overall local average EMR is 0.956 versus 0.221 for API models (averaged across GPT-4 and Claude Sonnet 4.5, the two API stacks with full Tasks~1--4 coverage; the four-stack average across all API entries in Table~1 is 0.319), meaning that fewer than one in four API output pairs are identical under conditions documented as deterministic.
...
The five API providers span a wide reproducibility range, with an 80-fold difference between the most and least reproducible (DeepSeek Chat extraction EMR~$=$~0.800 vs Perplexity Sonar extraction EMR~$=$~0.010; Fig.~\ref{fig:heatmap}).
...
To test this, we designed a quasi-isolation probe: we evaluated the same LLaMA~3 8B architecture served via Together AI's cloud endpoint (the INT4-quantised \texttt{Meta-Llama-3-8B-Instruct-Lite} endpoint) under identical prompts, seeds, and temperature as the local deployment.
...
Deterministic execution modes are technically feasible---batch-invariant inference kernels recover bitwise-identical outputs on open serving stacks\cite{he2025nondeterminism}---though they may entail performance trade-offs that current serving architectures prioritise differently.

Table~\ref{tab:mech_summary} summarises the mechanism-by-stack mapping at-a-glance; the full six-mechanism by five-stack breakdown with documentation-versus-inference labelling is in Methods Table~\ref{tab:mechanisms}.
...
Across all models and conditions, whole-output BERTScore F1 remains above 0.97 even when EMR approaches zero (at the finer per-field level it remains above 0.94; Supplementary Section~S14).
...
Non-determinism is therefore more than a technical curiosity: it interacts with how LLMs are deployed as measurement instruments, in a way that current evaluation frameworks do not account for.
...
We tested transfer to this setting with a focused out-of-domain probe: 10 PubMed abstracts on fine particulate matter (PM\textsubscript{2.5}) and respiratory health, drawn from the corpus of our companion paper\cite{rover2026evidence}, processed under our standard C1 structured-extraction condition (temperature~$=$~0, fixed seed where supported, five repetitions) across all eight deployment stacks with full coverage (full probe design and abstract DOIs in Supplementary Section~S12).

The pattern observed on AI/ML abstracts replicates with no qualitative change (Table~\ref{tab:revision_emr}).
...
API-Claude collapses to EMR~$=$~0.010 [0.00,\,0.03] on the 10 PM\textsubscript{2.5} abstracts---matching the 0.020 [0.00,\,0.05] observed on AI/ML summarisation and confirming that the deployment-stack signature of non-determinism is domain-transferable.

To probe whether the resulting textual divergence is cosmetic or substantive, we ran a blinded two-judge LLM-as-judge protocol on 30 newly sampled non-identical output pairs from the PM\textsubscript{2.5} corpus (18 produced by Claude Sonnet 4.5 and 12 by the OpenAI gpt-4.1 snapshot used in the companion paper---an external applied-context corpus, distinct from the gpt-4-0613 and gpt-4o stacks evaluated in the present study---stratified by disagreement kind (direction, magnitude, CI overlap, mixed) and distinct from the 23 study-level cases analysed in the companion paper).
Each pair was scored by two independent judges---Claude Opus 4.7 and gpt-4o---against three pre-registered criteria (direction of the reported effect, magnitude within $\pm$20\%, and overlap of confidence intervals) under a blind comparison setup, with verdicts \emph{truly\_contradictory}, \emph{semantically\_equivalent}, or \emph{ambiguous} (cost: USD~1.30; full prompts in Supplementary Section~S13).
...
Inter-judge agreement was 76.7\% (23 of 30 cases); Cohen's $\kappa$~=~0.29 (fair agreement on the Landis--Koch scale), a value depressed by the high prevalence of the truly-contradictory category (prevalence-adjusted bias-adjusted $\kappa$~=~0.53; Supplementary Section~S13). The judges differ chiefly on borderline cases that Claude assigns to ``ambiguous'' while gpt-4o commits to ``truly contradictory'': Claude's more conservative use of the ambiguous category (5 cases versus 0 for gpt-4o) accounts for the gap between the two truly-contradictory counts (22 versus 27), and the judges agree on the direction of effect.
...
The two in-paper layers above---the 10-abstract probe and the blinded two-judge protocol detailed in Supplementary Section~S13---are self-sufficient to support the propagation claim; the companion analysis is cited only as larger-corpus context, and Supplementary Section~S13.5 sets out exactly what is and is not evaluated in the present proof of concept relative to the full epidemiological systematic review.
We do not duplicate the companion paper's detailed pairwise reproducibility, silver-standard validation, and meta-analytic propagation tables here, citing instead its aggregate finding\cite{rover2026evidence} (a preregistered companion analysis, not yet peer-reviewed): across the full 500-abstract PM\textsubscript{2.5}-and-respiratory-health corpus, up to 23 study-level effect estimates appear or disappear from the evidence base depending solely on which run of the LLM is used.
...
\begin{figure}[t]
  ...
  \caption{\textbf{Exact match rates per deployment stack under greedy decoding reveal a three- to four-fold local--API gap.}
...
\begin{figure*}[t]
  ...
  The gpt-4o and DeepSeek multi-turn results (10 abstracts $\times$ 5 repetitions each) do not include RAG coverage; gpt-4o and DeepSeek bars therefore appear only in the multi-turn panel.
...
\begin{figure*}[t]
  ...
  \textbf{Right:} Key statistics: 7{,}004 experiments, 9 deployment stacks, 6 task families,
  3.1$\times$ local--API gap (balanced subsample), $<$1\% protocol overhead.
  }\label{fig:overview}
\end{figure*}
...
\medskip
{\footnotesize Examples~1--3 are drawn from the public Run Card repository; highlighting marks the diverging spans.}
\end{minipage}}
\end{figure*}
...
Under the settings that API documentation presents as deterministic (temperature zero, fixed seed), API-served models fail to reproduce their own outputs the majority of the time---between roughly two-thirds and four-fifths of paired runs, depending on the stack.
...
Recent work isolates a specific provider-side mechanism: He and Thinking Machines Lab attribute much of this non-determinism to the lack of \emph{batch invariance} in inference kernels---under varying server load the batch size, and hence the floating-point reduction order, changes from request to request---and show that batch-invariant kernels can restore bitwise-deterministic inference on open serving stacks\cite{he2025nondeterminism}.
...
Its key property is \emph{differential diagnosis}: when all hashes match except the output hash, the divergence is automatically attributed to the generation process, a result verified by the protocol-minimality ablation across 1{,}864 runs (Supplementary Section~S6) and consistent with the full 4{,}104-run analysis.
...
Input data for Tasks~1--4 comprises 30 English-language AI/ML abstracts; the study further covers standard public benchmarks (HumanEval, GSM8K) and 10 PubMed PM\textsubscript{2.5} abstracts as an out-of-AI/ML probe, with the same qualitative pattern observed in each.
Our main GPT-4 experiments used the \texttt{gpt-4-0613} snapshot, which may differ from current production behaviour; we partly mitigated this by running a current-snapshot subset (\texttt{gpt-4o-2024-11-20}) on the multi-turn extension, on HumanEval and GSM8K, and on the PubMed PM\textsubscript{2.5} probe, confirming that the qualitative local--API pattern persists at a current snapshot.
The multi-turn refinement task covers four API stacks (Claude, Gemini, gpt-4o, DeepSeek), but RAG coverage remains restricted to Claude and Gemini and extending the RAG regime to additional providers would further strengthen generality.
Sample sizes (10--30 problems per model per task) are well-powered for the large observed effects ($d$~$>$~1.6) but may miss subtler phenomena.
...
Our quasi-isolation experiment is consistent with production-infrastructure complexity, rather than the cloud medium, as the source.
...
\textbf{C-LLaMA3} = (LLaMA~3 8B Q4\_0 (INT4)\cite{grattafiori2024llama3} / Together AI / multi-GPU cloud / OpenAI-compatible API), pairing with L-LLaMA3 to isolate cloud serving from production-infrastructure complexity. Both L-LLaMA3 and C-LLaMA3 use the same INT4-quantised LLaMA~3 8B weights (local Ollama Q4\_0 and the Together AI \texttt{Lite} endpoint, respectively); they differ in serving infrastructure---single-GPU local execution versus a multi-GPU cloud cluster---whose per-mechanism consequences are detailed in Table~\ref{tab:mechanisms}.
...
Two further task families extend the evaluation to high-stakes domains where small textual differences alter functionality or numerical correctness:
...
A separate cross-domain probe (\textbf{Task~T14}, 10 PubMed abstracts on PM\textsubscript{2.5} and respiratory health drawn from the companion paper's 500-abstract corpus\cite{rover2026evidence}) was processed under Task~2's structured-extraction schema to test domain transfer outside AI/ML.
...
Tasks~5, 6, and T14 use established public benchmarks and a curated PubMed sample:
HumanEval problem identifiers (30 stratified) for Task~5, GSM8K test-split identifiers (30 sampled) for Task~6, and 10 PubMed abstracts with DOIs for Task~T14 (full lists in Supplementary Section~S2 (the 30 AI/ML abstracts), Section~S11 (HumanEval and GSM8K benchmark identifiers), and Section~S12 (PubMed PM\textsubscript{2.5} DOIs)).
...
Tasks~1--2 were run under all conditions for five models with full coverage; Tasks~3--4 (multi-turn refinement and RAG extraction) were run under C1 for the local stacks, API-Claude, and API-Gemini.
DeepSeek and Perplexity: C1 only on Tasks~1--2.
Together AI: C1 and C2 on Tasks~1--2.
Tasks~1--4 under these conditions yield 4{,}104 logged runs across 9 deployments and 7 execution environments.
Because API-served reproducibility is, by the argument of this paper, defined only relative to an infrastructure epoch, every Run Card records a UTC timestamp and we state the measurement window explicitly: Tasks~1--4 were collected in February 2026 (OpenAI stack: \texttt{gpt-4-0613}) and the code, mathematics, cross-domain, and multi-turn-extension batches in May 2026 (OpenAI stack: \texttt{gpt-4o-2024-11-20}). Per-provider windows are 2026-02-07 to 2026-05-09 for OpenAI and the local Ollama stacks, 2026-02-10 to 2026-05-09 for Anthropic, Google and DeepSeek, 2026-02-11 to 2026-05-09 for Together AI, and 2026-02-10 (single session) for Perplexity.

\textbf{Code, mathematics, and cross-domain tasks.} Three further batches were logged under C1, each with five repetitions per problem and the same Run Card / Prompt Card provenance discipline as Tasks~1--4.
...
A blinded two-judge LLM-as-judge analysis on 30 sampled non-identical pairs (18 Claude Sonnet 4.5 + 12 GPT-4.1) from Task~T14 was run with Claude Opus 4.7 and gpt-4o as the two judges under three pre-registered criteria (direction, magnitude $\pm$20\%, CI overlap); full prompts and per-pair verdicts are in Supplementary Section~S13.
These batches add 1{,}200 (HumanEval) + 1{,}200 (GSM8K) + 400 (PubMed PM\textsubscript{2.5}) + 100 (multi-turn extension) = 2{,}900 logged generative-AI runs, bringing the manuscript-wide total to 7{,}004 runs across 9 deployment stacks and 6 task families. An additional 60 LLM-as-judge calls (30 Claude Opus 4.7 + 30 gpt-4o, total cost USD~1.30, see Supplementary Section~S13, ``LLM-as-Judge Triangulation Protocol'') support the applied-impact triangulation but are reported separately as they belong to the validation layer rather than the primary experimental matrix.
...
The non-determinism we observe for the Claude stack therefore cannot arise from seed variation (no seed is transmitted); it reflects the production serving infrastructure.
...
Effect sizes: Cohen's $d$ for paired $t$-tests on EMR (parametric), Cliff's delta\cite{romano2006appropriate} for non-parametric rank-based comparisons, and Cohen's $h$ for Fisher's exact tests on perfect-reproducibility proportions (Supplementary Section~S9.5).
...
(5)~dynamic batching and continuous request scheduling, the lack of batch invariance recently isolated as a dominant cause\cite{kwon2023vllm,he2025nondeterminism};
...
(S10)~Experimental coverage matrix: runs per stack, task, and condition;
(S11)~Code and mathematics benchmarks: HumanEval problem identifiers, GSM8K problem identifiers, the gpt-4o-2024-11-20 multi-turn extension protocol, and full per-stack per-task EMR with 95\% bootstrap CIs;
(S12)~Cross-domain public-health probe: the 10 PubMed PM\textsubscript{2.5} abstract DOIs, task design, and the out-of-AI/ML extraction result;
(S13)~LLM-as-judge triangulation protocol: judge prompts, three pre-registered criteria (direction, magnitude $\pm$20\%, CI overlap), per-pair verdicts from two judges (Claude Opus 4.7: 22 truly\_contradictory / 3 semantically\_equivalent / 5 ambiguous; gpt-4o: 27 / 3 / 0; N=30), and cost log;
(S14)~Per-field reproducibility on the structured-extraction task (per-stack per-field EMR, NED, ROUGE-L, BERTScore).
...
All experimental records (4{,}104 runs for Tasks~1--4 together with the HumanEval, GSM8K, multi-turn-extension, and cross-domain PubMed batches), W3C PROV-JSON provenance documents, Run Cards, Prompt Cards, input data with DOIs, and source data for every figure and table are deposited on Figshare under a CC-BY~4.0 licence (persistent DOI \href{https://doi.org/10.6084/m9.figshare.31653373}{10.6084/m9.figshare.31653373}) and mirrored at the project's GitHub repository (\url{https://github.com/Roverlucas/genai-reproducibility-protocol}, release tag \texttt{v1.1}).

\item \textbf{Code availability:}
The full source code for the provenance protocol (Run Card, Prompt Card, W3C PROV-JSON generation), the experiment runners (\texttt{run\_experiments.py}, \texttt{run\_claude\_multiturn.py}, \texttt{run\_gemini\_multiturn.py}, \texttt{run\_chat\_control.py}, and a resumable batch orchestrator), the statistical and figure-generation pipeline, and the test suite (102 \texttt{pytest} tests) are publicly available under the MIT licence at \url{https://github.com/Roverlucas/genai-reproducibility-protocol} (release tag \texttt{v1.1}). A permanent mirror of the code archive is deposited on Figshare alongside the data with persistent DOI \href{https://doi.org/10.6084/m9.figshare.31653373}{10.6084/m9.figshare.31653373} (CC-BY 4.0). A \texttt{README.md} with installation and execution instructions and a pinned \texttt{requirements.txt} (Python~3.14.3) are provided.
\end{itemize}
...
\medskip\noindent\textbf{Table~\ref{tab:revision_emr} (below).}
Exact match rate per deployment stack on the code, mathematics, and cross-domain tasks: HumanEval (code generation), GSM8K (math reasoning), the multi-turn refinement extension to gpt-4o and DeepSeek, and the 10-abstract PubMed PM\textsubscript{2.5} cross-domain probe. Cells marked ``---'' indicate stacks not run on that task. 95\% bootstrap CIs from 10{,}000 percentile resamples.

% Force arabic numbering and continue from Methods Table 3 (sn-jnl appendices
% otherwise resets counter and renames format to \Alph{section}\arabic{table}).
...
\footnotesize
\setlength{\tabcolsep}{4pt}
\caption{\textbf{Exact match rate (EMR) with 95\% bootstrap CI per deployment stack on the code, mathematics, and cross-domain tasks.}
T5 = HumanEval (code); T6 = GSM8K (math); T3-ext = multi-turn refinement extended to gpt-4o and DeepSeek; T14 = 10 PubMed PM\textsubscript{2.5} abstracts (Task~2 extraction schema). 10\,000 percentile bootstrap resamples. The local--API effect size is reported as Cliff's $\delta$ (last row), pooling per-problem EMRs across the three local stacks versus the four API stacks for each task; $|\delta|{>}0.474$ corresponds to a large effect by Romano \emph{et~al.}\cite{romano2006appropriate}.}
