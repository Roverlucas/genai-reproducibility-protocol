...
\toprule
\textbf{Stack class} & \textbf{FP/quant arithmetic}\newline {\scriptsize (FP non-assoc., BF16/FP16)} & \textbf{Cloud-network factors}\newline {\scriptsize (latency, multi-tenancy, routing)} & \textbf{Production-serving complexity}\newline {\scriptsize (tensor par., FlashAttn., dyn.\ batching, spec.\ decoding)} \\
\midrule
Local (Ollama, single-GPU)        & mitigated (Q4 int) & ---            & ---                 \\
Together AI (cloud quasi-iso)     & active             & active         & mitigated by design \\
Closed APIs (GPT-4 / Claude / Gemini / DeepSeek / Perplexity) & likely active & active & likely active \\
\bottomrule
\end{tabular}%
}
\end{table}
