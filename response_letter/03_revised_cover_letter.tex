...
The companion paper (Rover \& Tadano, 2026) is currently in final preparation and reports the full 500-abstract PM\textsubscript{2.5}/respiratory-health analysis. To avoid duplicate publication, the present manuscript reports only the aggregate propagation finding (23 effects appear/disappear depending on which run is used) and the validation outcome, with full per-abstract data, silver-standard validation, and meta-analytic propagation residing in the companion paper, which is cited.

Given the strong methodological complementarity between the two manuscripts---this one establishing the hidden non-determinism phenomenon and a provenance protocol that detects it; the companion paper quantifying its empirical impact on a 500-abstract evidence base in environmental health---we would be very glad to make the companion manuscript available for parallel editorial consideration at \emph{Nature Communications}, should the editors wish to evaluate the two works as a coherent contribution. The companion paper has not yet been submitted to any journal, and we would welcome the opportunity to position the pair within the same editorial venue.
