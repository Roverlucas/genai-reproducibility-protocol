...
\begin{enumerate}
  ...
  \item \textbf{Applied evidence-synthesis impact and triangulation (R1.9; R3.6).} We promote the PM\textsubscript{2.5}/respiratory-health analysis to a dedicated Results subsection (``Applied impact in evidence synthesis''). For an in-paper validation of the divergence claim, we ran a two-judge LLM-as-judge protocol (Claude Opus~4.7 and gpt-4o, blind, three pre-registered criteria) on a new 30-case sample distinct from the 23 study-level effects of the companion paper; both judges classify the majority as truly contradictory (Claude 22/30, gpt-4o 27/30; inter-judge Cohen's $\kappa$~=~0.29). A complementary human-rater validation by two independent epidemiologists (including coauthor Y.\,S.\,T.) on a 10-case subsample, with Cohen's $\kappa$ reported, will accompany the resubmission package.
