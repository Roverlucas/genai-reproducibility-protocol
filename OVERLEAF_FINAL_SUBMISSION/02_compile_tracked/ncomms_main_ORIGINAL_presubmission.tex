...
The three local stacks (L-Gemma2, L-LLaMA3) and the Together AI quasi-isolation probe (C-LLaMA3) all achieve perfect EMR~$=$~1.000 [1.00,\,1.00]; L-Mistral attains 0.960 [0.88,\,1.00].
...
To probe whether the resulting textual divergence is cosmetic or substantive, we ran a blinded LLM-as-judge protocol on 10 newly sampled non-identical Claude output pairs from the PM\textsubscript{2.5} corpus---distinct from the 23 study-level cases analysed in the companion paper.
...
Revision total: 1{,}200 + 1{,}200 + 100 + 400 + 10 judge calls = 2{,}910 logged runs (rounded to 3{,}010 in the introduction together with the orchestration calls and provenance hash batches), bringing the manuscript-wide total to 7{,}114 logged generative-AI runs across 9 deployment stacks and 6 task families.
