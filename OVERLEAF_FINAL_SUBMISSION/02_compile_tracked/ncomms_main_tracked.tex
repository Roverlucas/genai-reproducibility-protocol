...
%DIF DEL ncomms_main_ORIGINAL_presubmission.tex   Mon May 25 17:37:24 2026
%DIF ADD ncomms_main_NEW_current.tex              Thu Jun 11 11:37:45 2026
...
\DIFdelbegin %DIFDELCMD < \affil[2]{\orgdiv{Research Group of Technology Applied to Optimization (GTAO)},
%DIFDELCMD < \orgname{Federal University of Paran\'a (UFPR)},
%DIFDELCMD < \orgaddress{\city{Curitiba}, \state{Paran\'a}, \country{Brazil}}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \affil[2]{\orgname{Federal University of Paran\'a (UFPR)},
\orgaddress{\country{Brazil}}}
\DIFaddend 

\DIFdelbegin %DIFDELCMD < \affil[3]{\orgdiv{School of Applied Sciences (FCA)},
%DIFDELCMD < \orgname{University of Campinas (Unicamp)},
%DIFDELCMD < \orgaddress{\city{Limeira}, \state{S\~ao Paulo}, \country{Brazil}}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \affil[3]{\orgname{University of Campinas (Unicamp)},
\orgaddress{\country{Brazil}}}
\DIFaddend 
...
\DIFdelbegin %DIFDELCMD < \abstract{Large language model APIs serve research in medicine, climate science, and engineering under settings documented as deterministic. We show this guarantee fails in practice. Across 7{,}004 controlled experiments on nine \emph{deployment stacks}---tuples of (weights, provider, infrastructure, API)---and six task families (extraction, summarisation, multi-turn refinement, RAG, code, math), API-served stacks reproduce their own outputs as little as 1\% of the time under temperature-zero greedy decoding with fixed seeds, while local stacks reach 92--100\%. The variation is invisible: outputs remain semantically equivalent (BERTScore F1~$>$~0.97) while differing textually. The gap widens in high-stakes regimes---multi-turn dialogue, RAG, code, and math---where small differences alter functionality. A quasi-isolation experiment with identical LLaMA~3~8B weights served by two stacks attributes the cause to production infrastructure complexity rather than cloud deployment itself. We provide a W3C PROV-based provenance protocol ($<$1\% overhead) that makes this hidden variation detectable and auditable.}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \abstract{Large language model APIs serve research in medicine, climate science, and engineering under settings documented as deterministic. We show this guarantee fails in practice. Across 7{,}004 experiments on nine \emph{deployment stacks} (weights, provider, infrastructure, API) and six task families (extraction, summarisation, multi-turn refinement, RAG, code, math), API-served stacks reproduce their own outputs as little as 1\% under temperature-zero greedy decoding with fixed seeds, while local-stack averages reach 93--98\%. The variation is invisible: outputs appear semantically equivalent (BERTScore F1~$>$~0.97) but diverge textually, yet two blinded LLM judges (Claude Opus 4.7, gpt-4o) classify the majority (73--90\%) as truly contradictory. The gap persists in high-stakes regimes (multi-turn, RAG, code, math), widening further on the worst-case API stacks. A quasi-isolation experiment with identical LLaMA~3~8B weights on two stacks shows that cloud deployment alone does not preclude reproducibility, pointing to production-infrastructure complexity rather than the cloud medium. We release a W3C PROV-based provenance protocol ($<$1\% overhead) making this hidden variation auditable.}
\DIFaddend 
...
We conducted \DIFdelbegin \DIFdel{4,104 controlled experiments in the original analysis and an additional 2,900 experiments in revision (}\DIFdelend 7\DIFdelbegin \DIFdel{,}\DIFdelend \DIFaddbegin {\DIFadd{,}}\DIFaddend 004 \DIFdelbegin \DIFdel{in total) }\DIFdelend \DIFaddbegin \DIFadd{controlled experiments }\DIFaddend across nine deployment stacks, eight models \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(counting OpenAI's GPT-4 family---the gpt-4-0613 and gpt-4o-2024-11-20 snapshots---as a single model, and LLaMA~3 8B once despite being served on two stacks), }\DIFaddend five independent API providers, and six task families\DIFdelbegin \DIFdel{spanning }\DIFdelend \DIFaddbegin \DIFadd{: }\DIFaddend structured extraction, scientific summarisation, multi-turn refinement, retrieval-augmented generation, code generation (HumanEval), and mathematical reasoning (GSM8K).
We found that even under temperature~$=$~0 with fixed random seeds, API-served models reproduce their own outputs only 22.1\% of the time on \DIFdelbegin \DIFdel{the original two tasks, while locally }\DIFdelend \DIFaddbegin \DIFadd{Tasks~1--2 (averaged across GPT-4 and Claude Sonnet 4.5, the two API stacks with full Tasks~1--4 coverage). Locally }\DIFaddend deployed open-weight models achieve 95.6\% exact \DIFdelbegin \DIFdel{reproducibility---a gap exceeding four-fold (}\DIFdelend \DIFaddbegin \DIFadd{reproducibility on the same tasks---a ratio of approximately $4\times$ in point estimates ($3.1\times$ under the balanced 10-abstract subsample, }\DIFaddend 95\% bootstrap CI\DIFdelbegin \DIFdel{for the ratio}\DIFdelend : 2.48--3.61$\times$\DIFdelbegin \DIFdel{) that the revision experiments confirm }\DIFdelend \DIFaddbegin \DIFadd{; Extended Data Table~4), and the gap }\DIFaddend extends to code, math, and out-of-AI/ML domains.
...
Here, we make \DIFdelbegin \DIFdel{four contributions.
First, we reveal and quantify hidden non-determinism }\DIFdelend \DIFaddbegin \DIFadd{five contributions.
While individual reports have noted that ``deterministic'' API settings can fail\mbox{%DIFAUXCMD
\cite{atil2024nondeterminism,ouyang2024nondeterminism}}\hskip0pt%DIFAUXCMD
, our first contribution is to quantify the phenomenon systematically: a controlled local-versus-API contrast at bitwise (SHA-256) resolution }\DIFaddend across all five major API providers tested (OpenAI, Anthropic, Google, DeepSeek, Perplexity) \DIFdelbegin \DIFdel{, showing that it is }\DIFdelend \DIFaddbegin \DIFadd{and six task families, establishing it as }\DIFaddend a general property of cloud-served LLM APIs rather than a \DIFdelbegin \DIFdel{provider-specific }\DIFdelend \DIFaddbegin \DIFadd{provider- or task-specific }\DIFaddend anomaly.
...
Third, we demonstrate that cloud deployment \emph{per se} does not cause non-determinism: the same LLaMA~3 8B weights served via two stacks (local Ollama versus Together AI's cloud endpoint) achieve near-identical exact match rates\DIFdelbegin \DIFdel{, implicating }\DIFdelend \DIFaddbegin \DIFadd{. This is consistent with }\DIFaddend production-infrastructure \DIFdelbegin \DIFdel{complexity (tensor }\DIFdelend \DIFaddbegin \DIFadd{complexity---tensor }\DIFaddend parallelism, speculative decoding, dynamic \DIFdelbegin \DIFdel{batching) rather }\DIFdelend \DIFaddbegin \DIFadd{batching---rather }\DIFaddend than the cloud medium \DIFdelbegin \DIFdel{and confirming }\DIFdelend \DIFaddbegin \DIFadd{itself, and indicates }\DIFaddend that the deployment stack, not the model in isolation, is the carrier of variation.
...
Fifth, we }\DIFaddend provide a lightweight provenance protocol grounded in the W3C Provenance (PROV) Data Model\cite{w3cprov2013}---a W3C Recommendation that defines an interoperable standard for representing the entities, activities, and agents involved in producing a piece of data---and extending Model Cards\cite{mitchell2019model} and Datasheets\cite{gebru2021datasheets} to the inference layer, that adds less than 1\% overhead and makes this invisible variation visible, auditable, and attributable.
...
Under the same greedy decoding\DIFdelbegin \DIFdel{with fixed seeds}\DIFdelend , GPT-4 achieves EMR~$=$~0.443 [0.32,\,0.57] for extraction and 0.230 [0.16,\,0.30] for \DIFdelbegin \DIFdel{summarization}\DIFdelend \DIFaddbegin \DIFadd{summarisation (variable-seed condition C2, as the OpenAI seed parameter is only advisory; the seed-free local and Claude stacks use C1; see Methods)}\DIFaddend .
...
Deterministic execution modes are technically \DIFdelbegin \DIFdel{feasible, though }\DIFdelend \DIFaddbegin \DIFadd{feasible---batch-invariant inference kernels recover bitwise-identical outputs on open serving stacks\mbox{%DIFAUXCMD
\cite{he2025nondeterminism}}\hskip0pt%DIFAUXCMD
---though }\DIFaddend they may entail performance trade-offs that current serving architectures \DIFdelbegin \DIFdel{prioritize }\DIFdelend \DIFaddbegin \DIFadd{prioritise }\DIFaddend differently.

\DIFaddbegin \DIFadd{Table~\ref{tab:mech_summary} summarises the mechanism-by-stack mapping at-a-glance; the full six-mechanism by five-stack breakdown with documentation-versus-inference labelling is in Methods Table~\ref{tab:mechanisms}.
}


\DIFaddend \subsection{Complex workflows amplify the reproducibility gap}\label{subsec:multiturn}
...
First, \DIFdelbegin \DIFdel{the local-to-API gap widens, not narrows, }\DIFdelend on tasks where reproducibility is most consequential \DIFaddbegin \DIFadd{the worst-case API stack becomes substantially less reproducible, not more}\DIFaddend : the smallest API EMR observed on HumanEval and GSM8K (0.393 and 0.063, respectively, both for the Claude stack) is the smallest single-task EMR reported anywhere in this manuscript. \DIFaddbegin \DIFadd{The aggregate Cliff's $\delta$ on HumanEval is $+0.266$ (small)---smaller than the $\delta$ on Tasks~1--2 (0.784--0.896, large)---because API-Gemini achieves perfect EMR (1.000) on this structured benchmark; the local--API gap is therefore driven by the worst-case API stacks rather than uniformly distributed across all API stacks, an observation that strengthens rather than weakens the deployment-stack framing.
...
Across all models and conditions, \DIFaddbegin \DIFadd{whole-output }\DIFaddend BERTScore F1 remains above 0.97 even when EMR approaches zero \DIFaddbegin \DIFadd{(at the finer per-field level it remains above 0.94; Supplementary Section~S13)}\DIFaddend .
...
Non-determinism is therefore \DIFdelbegin \DIFdel{not }\DIFdelend \DIFaddbegin \DIFadd{more than }\DIFaddend a technical curiosity\DIFdelbegin \DIFdel{but a property that }\DIFdelend \DIFaddbegin \DIFadd{: it }\DIFaddend interacts with how LLMs are deployed as measurement instruments, \DIFdelbegin \DIFdel{one }\DIFdelend \DIFaddbegin \DIFadd{in a way }\DIFaddend that current evaluation frameworks do not account for.
...
The pattern observed on AI/ML abstracts replicates with no qualitative change (\DIFdelbegin \DIFdel{Extended Data }\DIFdelend Table~\ref{tab:revision_emr}).
\DIFdelbegin \DIFdel{The three }\DIFdelend \DIFaddbegin \DIFadd{Two }\DIFaddend local stacks (L-Gemma2 \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend L-LLaMA3) and the Together AI quasi-isolation probe (C-LLaMA3) all achieve perfect EMR~$=$~1.000 [1.00,\,1.00]; L-Mistral attains 0.960 [0.88,\,1.00].
API-DeepSeek reaches 0.660 [0.46,\,0.85], API-Gemini 0.490 [0.26,\,0.72], and API-GPT4 0.420 [0.27,\,0.58].
API-Claude collapses to EMR~$=$~0.010 [0.00,\,0.03] on the 10 PM\textsubscript{2.5} abstracts---matching the 0.020 [0.00,\,0.05] observed on AI/ML summarisation \DIFdelbegin \DIFdel{in our original analysis }\DIFdelend and confirming that the deployment-stack signature of non-determinism is domain-transferable.

To probe whether the resulting textual divergence is cosmetic or substantive, we ran a blinded \DIFaddbegin \DIFadd{two-judge }\DIFaddend LLM-as-judge protocol on \DIFdelbegin \DIFdel{10 }\DIFdelend \DIFaddbegin \DIFadd{30 }\DIFaddend newly sampled non-identical \DIFdelbegin \DIFdel{Claude }\DIFdelend output pairs from the PM\textsubscript{2.5} \DIFdelbegin \DIFdel{corpus---distinct from the }\DIFdelend \DIFaddbegin \DIFadd{corpus (18 produced by Claude Sonnet 4.5 and 12 by the OpenAI gpt-4.1 snapshot used in the companion paper---an external applied-context corpus, distinct from the gpt-4-0613 and gpt-4o stacks evaluated in the present study---stratified by disagreement kind (direction, magnitude, CI overlap, mixed) and distinct from the }\DIFaddend 23 study-level cases analysed in the companion paper\DIFaddbegin \DIFadd{)}\DIFaddend .
Each pair was scored by \DIFdelbegin \DIFdel{Claude }\DIFdelend \DIFaddbegin \DIFadd{two independent judges---Claude }\DIFaddend Opus 4.7 \DIFdelbegin \DIFdel{against }\DIFdelend \DIFaddbegin \DIFadd{and gpt-4o---against }\DIFaddend three pre-registered criteria (direction of the reported effect, magnitude within $\pm$20\%, and overlap of confidence intervals) under a blind comparison setup, with verdicts \emph{truly\_contradictory}, \emph{semantically\_equivalent}, or \emph{ambiguous} (cost: USD~\DIFdelbegin \DIFdel{0.28}\DIFdelend \DIFaddbegin \DIFadd{1.30}\DIFaddend ; full prompts in Supplementary Section~S12).
\DIFdelbegin \DIFdel{The judge classified 5 of the 10 }\DIFdelend \DIFaddbegin \DIFadd{Claude Opus 4.7 classified 22 of the 30 }\DIFaddend pairs as truly contradictory \DIFaddbegin \DIFadd{(Wilson 95\% CI 56--86\%)}\DIFaddend , 3 as semantically equivalent \DIFdelbegin \DIFdel{, and 2 as ambiguous .
Half of the }\DIFdelend \DIFaddbegin \DIFadd{(3--26\%), and 5 as ambiguous (7--34\%). gpt-4o classified 27 as truly contradictory (74--97\%) and 3 as semantically equivalent (3--26\%), using none of the ambiguous category.
Inter-judge agreement was 76.7\% (23 of 30 cases); Cohen's $\kappa$~=~0.29 (fair agreement on the Landis--Koch scale), a value depressed by the high prevalence of the truly-contradictory category (prevalence-adjusted bias-adjusted $\kappa$~=~0.53; Supplementary Section~S12). The judges differ chiefly on borderline cases that Claude assigns to ``ambiguous'' while gpt-4o commits to ``truly contradictory'': Claude's more conservative use of the ambiguous category (5 cases versus 0 for gpt-4o) accounts for the gap between the two truly-contradictory counts (22 versus 27), and the judges agree on the direction of effect.
...
The majority of the }\DIFaddend divergences in the same \DIFdelbegin \DIFdel{Claude }\DIFdelend outputs that show BERTScore F1~$>$~0.97 are therefore substantive disagreements about the underlying scientific claim, not surface-level rephrasings: BERTScore alone cannot tell a cosmetic variation from a reversed direction of effect or a confidence interval that no longer overlaps the original.

We do not duplicate the companion paper's detailed pairwise reproducibility, silver-standard validation, and meta-analytic propagation tables here, citing instead \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend aggregate finding\cite{rover2026evidence} \DIFaddbegin \DIFadd{(a preregistered companion analysis, not yet peer-reviewed)}\DIFaddend : across the full 500-abstract PM\textsubscript{2.5}-and-respiratory-health corpus, up to 23 study-level effect estimates appear or disappear from the evidence base depending solely on which run of the LLM is used.
...
\begin{figure}[t]
  ...
  \caption{\DIFdelbeginFL \textbf{\DIFdelFL{Exact match rates per deployment stack under greedy decoding reveal a four-fold local--API gap.}}
  %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Exact match rates per deployment stack under greedy decoding reveal a three- to four-fold local--API gap.}}
  \DIFaddendFL Heatmap of EMR across extraction and \DIFdelbeginFL \DIFdelFL{summarization }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{summarisation }\DIFaddendFL tasks under temperature~$=$~0
...
%DIF > % --- Table 2: At-a-glance mechanism activation (single column) ---
\DIFaddbegin \begin{table}[h]
\caption{\textbf{\DIFaddFL{At-a-glance mechanism activation by deployment-stack class.}} \DIFaddFL{Together AI strips the production-serving cluster while retaining cloud-network and FP arithmetic factors, isolating production-serving complexity as the residual driver of variation in closed APIs. See Methods Table~\ref{tab:mechanisms} for the per-mechanism, per-stack breakdown.}}\label{tab:mech_summary}
\footnotesize
\centering
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}p{3.2cm}p{2.8cm}p{2.5cm}p{3.0cm}@{}}
\toprule
\textbf{\DIFaddFL{Stack class}} & \textbf{\DIFaddFL{FP/quant arithmetic\textsuperscript{\,a}}} & \textbf{\DIFaddFL{Cloud-network factors\textsuperscript{\,b}}} & \textbf{\DIFaddFL{Production-serving complexity\textsuperscript{\,c}}} \\
\midrule
\DIFaddFL{Local (Ollama, single-GPU)        }& \DIFaddFL{mitigated (Q4 int) }& \DIFaddFL{---            }& \DIFaddFL{---                 }\\
\DIFaddFL{Together AI (cloud quasi-iso)     }& \DIFaddFL{active             }& \DIFaddFL{active         }& \DIFaddFL{mitigated by design }\\
\DIFaddFL{Closed APIs\textsuperscript{\,*}  }& \DIFaddFL{likely active      }& \DIFaddFL{active         }& \DIFaddFL{likely active       }\\
\bottomrule
\end{tabular}
\par\vspace{4pt}
{\scriptsize \DIFaddFL{``mitigated'' = single-GPU integer-arithmetic execution or quasi-isolation by design; ``active'' = documented or strongly likely from public information. \textsuperscript{a}FP non-associativity, BF16/FP16 mixed-precision; \textsuperscript{b}latency, multi-tenancy, request routing; \textsuperscript{c}tensor parallelism, FlashAttention, dynamic batching, speculative decoding; \textsuperscript{*}GPT-4, Claude, Gemini, DeepSeek, Perplexity.}}
\end{table}

\DIFaddend %% --- Figure 2: Multi-turn + RAG (full width) ---
\begin{figure*}[t]
  \centering
  ...
  The gpt-4o and DeepSeek multi-turn results (10 abstracts $\times$ 5 repetitions each) \DIFdelbeginFL \DIFdelFL{were added in revision per reviewer request R3.3 and }\DIFdelendFL do not \DIFdelbeginFL \DIFdelFL{have }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{include }\DIFaddendFL RAG coverage; gpt-4o and DeepSeek bars therefore appear only in the multi-turn panel.
...
Under the settings that API documentation presents as deterministic (temperature zero, fixed seed), API-served models fail to reproduce their own outputs \DIFdelbegin \DIFdel{approximately four out of five times}\DIFdelend \DIFaddbegin \DIFadd{the majority of the time---between roughly two-thirds and four-fifths of paired runs, depending on the stack}\DIFaddend .
...
\DIFaddbegin \DIFadd{Recent work isolates a specific provider-side mechanism: He and Thinking Machines Lab attribute much of this non-determinism to the lack of }\emph{\DIFadd{batch invariance}} \DIFadd{in inference kernels---under varying server load the batch size, and hence the floating-point reduction order, changes from request to request---and show that batch-invariant kernels can restore bitwise-deterministic inference on open serving stacks\mbox{%DIFAUXCMD
\cite{he2025nondeterminism}}\hskip0pt%DIFAUXCMD
.
That result is complementary to ours and strengthens our interpretation: it identifies the engineering lever (dynamic batching, our mechanism~5) on stacks where the kernels are user-accessible, whereas our contribution is (i)~the first cross-provider quantification of the resulting irreproducibility across closed commercial APIs, where such kernels are not exposed to the user, and (ii)~a client-side provenance protocol that makes the variation auditable without provider cooperation. It also sharpens our feasibility point (Results): bitwise-deterministic execution is demonstrably achievable, so the non-determinism we document is a design choice rather than an inevitability.
}\DIFaddend Our study advances beyond these observations by systematically comparing local versus API deployments across five providers, extending measurements to multi-turn and RAG workflows, and providing a causal attribution framework that distinguishes infrastructure complexity from cloud deployment.

The Together AI quasi-isolation result provides a key mechanistic insight: because the same model architecture achieves near-local reproducibility via a different cloud API, the non-determinism observed in GPT-4, Claude, and Gemini is \DIFdelbegin \DIFdel{attributable to }\DIFdelend \DIFaddbegin \DIFadd{consistent with }\DIFaddend production serving complexity rather than cloud deployment \DIFdelbegin \DIFdel{itself}\DIFdelend \DIFaddbegin \DIFadd{itself---with the caveat that model scale and serving complexity are confounded within our single open-weight pair (see Limitations), so the probe is best read as an existence proof that cloud deployment is not a sufficient cause}\DIFaddend .
...
Input data for \DIFdelbegin \DIFdel{the original }\DIFdelend Tasks~1--4 comprises 30 English-language AI/ML abstracts; the \DIFdelbegin \DIFdel{revision extends to }\DIFdelend \DIFaddbegin \DIFadd{study further covers }\DIFaddend standard public benchmarks (HumanEval, GSM8K) and \DIFdelbegin \DIFdel{to }\DIFdelend 10 PubMed PM\textsubscript{2.5} abstracts as an out-of-AI/ML probe, with the same qualitative pattern observed in each.
\DIFdelbegin \DIFdel{The original }\DIFdelend \DIFaddbegin \DIFadd{Our main }\DIFaddend GPT-4 experiments used the \texttt{gpt-4-0613} snapshot, which may differ from current production behaviour; we partly mitigated this by running a current-snapshot subset (\texttt{gpt-4o-2024-11-20}) on the multi-turn extension, on HumanEval and GSM8K, and on the PubMed PM\textsubscript{2.5} probe, confirming that the qualitative local--API pattern persists at a current snapshot.
The multi-turn refinement task \DIFdelbegin \DIFdel{now }\DIFdelend covers four API stacks (Claude, Gemini, gpt-4o, DeepSeek), but RAG coverage remains restricted to Claude and Gemini and extending the RAG regime to additional providers would further strengthen generality.
Sample sizes (10--30 problems per model per \DIFdelbegin \DIFdel{revision }\DIFdelend task) are well-powered for the large observed effects ($d$~$>$~1.6) but may miss subtler phenomena.
...
\DIFdelbegin \DIFdel{Indeed, preliminary evidence }\DIFdelend \DIFaddbegin \DIFadd{Evidence }\DIFaddend from an applied domain \DIFdelbegin \DIFdel{suggests }\DIFdelend \DIFaddbegin \DIFadd{confirms }\DIFaddend that the effects reported here have direct consequences for scientific conclusions.
...
Critically, a }\DIFdelend \DIFaddbegin \DIFadd{on the 10-abstract probe (where Claude EMR collapses further to 0.010 because the smaller corpus concentrates the within-abstract variance; the two values therefore quantify the same phenomenon at different sampling scales).
A }\DIFaddend meta-analytic propagation analysis \DIFdelbegin \DIFdel{reveals that }\DIFdelend \DIFaddbegin \DIFadd{in that preregistered companion study (not yet peer-reviewed) indicates that }\DIFaddend up to 23 study-level effect estimates---each representing a distinct epidemiological finding on air pollution and respiratory outcomes---appear or disappear from the evidence base depending solely on which run of the LLM is used.
...
Our quasi-isolation experiment \DIFdelbegin \DIFdel{attributes the cause to production infrastructure complexityrather than cloud deployment itself}\DIFdelend \DIFaddbegin \DIFadd{is consistent with production-infrastructure complexity, rather than the cloud medium, as the source}\DIFaddend .
...
The first four \DIFdelbegin \DIFdel{were evaluated in the original analysis:
...
Two \DIFdelbegin \DIFdel{additional task families were added in revision}\DIFdelend \DIFaddbegin \DIFadd{further task families extend the evaluation to high-stakes domains where small textual differences alter functionality or numerical correctness}\DIFaddend :
...
\DIFdelbegin \DIFdel{Revision tasks }\DIFdelend \DIFaddbegin \DIFadd{Tasks~5, 6, and T14 }\DIFaddend use established public benchmarks and a curated PubMed sample:
HumanEval problem identifiers (30 stratified) for Task~5, GSM8K test-split identifiers (30 sampled) for Task~6, and 10 PubMed abstracts with DOIs for Task~T14 (full lists and DOIs in Supplementary Section~S2 \DIFdelbegin \DIFdel{and the new Section~S2-bis)}\DIFdelend \DIFaddbegin \DIFadd{(the 30 AI/ML abstracts) and Section~S11 (benchmark and PubMed identifiers))}\DIFaddend .
...
Tasks~1--2 \DIFaddbegin \DIFadd{were run }\DIFaddend under all conditions for five models with full coverage; Tasks~3--4 \DIFaddbegin \DIFadd{(multi-turn refinement and RAG extraction) were run }\DIFaddend under C1 for \DIFdelbegin \DIFdel{local models, Claude, and Gemini}\DIFdelend \DIFaddbegin \DIFadd{the local stacks, API-Claude, and API-Gemini}\DIFaddend .
DeepSeek and Perplexity: C1 only on Tasks~1--2\DIFdelbegin \DIFdel{in the original analysis}\DIFdelend .
Together AI: C1 and C2 on Tasks~1--2.
\DIFdelbegin \DIFdel{Grand total for the original analysis: }\DIFdelend \DIFaddbegin \DIFadd{Tasks~1--4 under these conditions yield }\DIFaddend 4\DIFdelbegin \DIFdel{,}\DIFdelend \DIFaddbegin {\DIFadd{,}}\DIFaddend 104 logged runs across 9 deployments and 7 execution environments.
\DIFaddbegin \DIFadd{Because API-served reproducibility is, by the argument of this paper, defined only relative to an infrastructure epoch, every Run Card records a UTC timestamp and we state the measurement window explicitly: Tasks~1--4 were collected in February 2026 (OpenAI stack: }\texttt{\DIFadd{gpt-4-0613}}\DIFadd{) and the code, mathematics, cross-domain, and multi-turn-extension batches in May 2026 (OpenAI stack: }\texttt{\DIFadd{gpt-4o-2024-11-20}}\DIFadd{). Per-provider windows are 2026-02-07 to 2026-05-09 for OpenAI and the local Ollama stacks, 2026-02-10 to 2026-05-09 for Anthropic, Google and DeepSeek, 2026-02-11 to 2026-05-09 for Together AI, and 2026-02-10 (single session) for Perplexity.
}\DIFaddend 

\DIFdelbegin \textbf{\DIFdel{Revision extensions.}} %DIFAUXCMD
\DIFdel{Following reviewer requests during the major revision (R3.3, R3.4, R3.6), three additional }\DIFdelend \DIFaddbegin \textbf{\DIFadd{Code, mathematics, and cross-domain tasks.}} \DIFadd{Three further }\DIFaddend batches were logged under C1, each with five repetitions per problem and the same Run Card / Prompt Card provenance discipline as \DIFdelbegin \DIFdel{the original runs}\DIFdelend \DIFaddbegin \DIFadd{Tasks~1--4}\DIFaddend .
...
A blinded \DIFaddbegin \DIFadd{two-judge }\DIFaddend LLM-as-judge analysis on \DIFdelbegin \DIFdel{10 }\DIFdelend \DIFaddbegin \DIFadd{30 }\DIFaddend sampled non-identical \DIFdelbegin \DIFdel{Claude pairs }\DIFdelend \DIFaddbegin \DIFadd{pairs (18 Claude Sonnet 4.5 + 12 GPT-4.1) }\DIFaddend from Task~T14 was run with Claude Opus 4.7 \DIFdelbegin \DIFdel{as the judge }\DIFdelend \DIFaddbegin \DIFadd{and gpt-4o as the two judges }\DIFaddend under three pre-registered criteria (direction, magnitude $\pm$20\%, CI overlap); full prompts and per-pair verdicts are in Supplementary Section~S12.
\DIFdelbegin \DIFdel{Revision total: }\DIFdelend \DIFaddbegin \DIFadd{These batches add }\DIFaddend 1{,}200 \DIFaddbegin \DIFadd{(HumanEval) }\DIFaddend + 1{,}200 \DIFaddbegin \DIFadd{(GSM8K) }\DIFaddend + \DIFdelbegin \DIFdel{100 + }\DIFdelend 400 \DIFaddbegin \DIFadd{(PubMed PM\textsubscript{2.5}) }\DIFaddend + \DIFdelbegin \DIFdel{10 judge calls }\DIFdelend \DIFaddbegin \DIFadd{100 (multi-turn extension) }\DIFaddend = 2{,}\DIFdelbegin \DIFdel{910 logged runs (rounded to 3}%DIFDELCMD < {%%%
\DIFdel{, }%DIFDELCMD < }%%%
\DIFdel{010 in the introduction together with the orchestration calls and provenance hash batches), }\DIFdelend \DIFaddbegin \DIFadd{900 logged generative-AI runs, }\DIFaddend bringing the manuscript-wide total to 7{,}\DIFdelbegin \DIFdel{114 logged generative-AI }\DIFdelend \DIFaddbegin \DIFadd{004 }\DIFaddend runs across 9 deployment stacks and 6 task families. \DIFaddbegin \DIFadd{An additional 60 LLM-as-judge calls (30 Claude Opus 4.7 + 30 gpt-4o, total cost USD~1.30, see Supplementary Section~S12, ``LLM-as-Judge Triangulation Protocol'') support the applied-impact triangulation but are reported separately as they belong to the validation layer rather than the primary experimental matrix.
}\DIFaddend 
...
The non-determinism we observe for the Claude stack \DIFdelbegin \DIFdel{is therefore attributable to }\DIFdelend \DIFaddbegin \DIFadd{therefore cannot arise from seed variation (no seed is transmitted); it reflects }\DIFaddend the production serving infrastructure\DIFdelbegin \DIFdel{rather than to seed variation}\DIFdelend .
...
(5)~dynamic batching and continuous request scheduling\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{kwon2023vllm}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{, the lack of batch invariance recently isolated as a dominant cause\mbox{%DIFAUXCMD
\cite{kwon2023vllm,he2025nondeterminism}}\hskip0pt%DIFAUXCMD
}\DIFaddend ;
...
%DIFDELCMD < \toprule
%DIFDELCMD < %%%
\textbf{\DIFdelFL{Mechanism}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{L-LLaMA3}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{C-LLaMA3 (Together)}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{API-GPT4}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{API-Claude}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{API-Gemini}} %DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD < \midrule
%DIFDELCMD < %%%
\DIFdelFL{Non-associative FP arithmetic     }%DIFDELCMD < & %%%
\DIFdelFL{$\triangle$ }%DIFDELCMD < & %%%
\DIFdelFL{$\triangle$ }%DIFDELCMD < & %%%
\DIFdelFL{$\triangle$ }%DIFDELCMD < & %%%
\DIFdelFL{$\triangle$ }%DIFDELCMD < & %%%
\DIFdelFL{$\triangle$ }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{Mixed-precision BF16/FP16         }%DIFDELCMD < & %%%
\DIFdelFL{--- (Q4 int)}%DIFDELCMD < & \checkmark  & %%%
\DIFdelFL{likely      }%DIFDELCMD < & %%%
\DIFdelFL{likely      }%DIFDELCMD < & %%%
\DIFdelFL{likely }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{Tensor parallelism                }%DIFDELCMD < & %%%
\DIFdelFL{---         }%DIFDELCMD < & \checkmark  & %%%
\DIFdelFL{likely      }%DIFDELCMD < & %%%
\DIFdelFL{likely      }%DIFDELCMD < & %%%
\DIFdelFL{likely }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{FlashAttention kernel             }%DIFDELCMD < & %%%
\DIFdelFL{---         }%DIFDELCMD < & %%%
\DIFdelFL{likely      }%DIFDELCMD < & %%%
\DIFdelFL{likely      }%DIFDELCMD < & %%%
\DIFdelFL{likely      }%DIFDELCMD < & %%%
\DIFdelFL{likely }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{Dynamic batching                  }%DIFDELCMD < & %%%
\DIFdelFL{---         }%DIFDELCMD < & \checkmark  & %%%
\DIFdelFL{likely      }%DIFDELCMD < & %%%
\DIFdelFL{likely      }%DIFDELCMD < & %%%
\DIFdelFL{likely }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{Speculative decoding              }%DIFDELCMD < & %%%
\DIFdelFL{---         }%DIFDELCMD < & %%%
\DIFdelFL{possible    }%DIFDELCMD < & %%%
\DIFdelFL{likely      }%DIFDELCMD < & %%%
\DIFdelFL{likely      }%DIFDELCMD < & %%%
\DIFdelFL{likely }%DIFDELCMD < \\
...
All run records (JSON), PROV-JSON provenance documents, Run Cards, Prompt Cards, input data (30 ML/AI abstracts with DOIs, 10 PubMed \DIFdelbegin \DIFdel{PM2.5 }\DIFdelend \DIFaddbegin \DIFadd{PM\textsubscript{2.5} }\DIFaddend abstracts, HumanEval and GSM8K problem identifiers), and source data underlying every figure and table are \DIFdelbegin \DIFdel{publicly available at }%DIFDELCMD < \url{https://github.com/Roverlucas/genai-reproducibility-protocol} %%%
\DIFdelend \DIFaddbegin \DIFadd{deposited on Figshare }\DIFaddend under a CC-BY~4.0 licence \DIFdelbegin \DIFdel{. An archived snapshot of the release tag }\DIFdelend \DIFaddbegin \DIFadd{(DOI: }\href{https://doi.org/10.6084/m9.figshare.31653373}{\DIFadd{10.6084/m9.figshare.31653373}}\DIFadd{). The Figshare deposit mirrors the project GitHub repository at }\url{https://github.com/Roverlucas/genai-reproducibility-protocol} \DIFadd{(release tag }\DIFaddend \texttt{v1.1\DIFdelbegin \DIFdel{-natcomms-revision1}\DIFdelend }\DIFdelbegin \DIFdel{will be deposited in Zenodo on acceptance and a persistent DOI will be inserted at proof stage}\DIFdelend \DIFaddbegin \DIFadd{)}\DIFaddend . Source data are provided with this paper.

\bmhead{Code availability}

The reference implementation of the provenance protocol (Run Card, Prompt Card, W3C PROV-JSON generation), all experiment runners \DIFdelbegin \DIFdel{, the statistical and figure-generation pipeline, and the revision extensions }\DIFdelend \DIFaddbegin \DIFadd{(including those }\DIFaddend for HumanEval, GSM8K and the cross-domain PubMed probe\DIFaddbegin \DIFadd{), and the statistical and figure-generation pipeline }\DIFaddend are publicly available at \url{https://github.com/Roverlucas/genai-reproducibility-protocol} under the MIT licence \DIFaddbegin \DIFadd{(release tag }\texttt{\DIFadd{v1.1}}\DIFadd{)}\DIFaddend . The repository includes a \texttt{README.md} with installation and execution instructions, pinned dependencies (\texttt{requirements.txt}, Python 3.14.3), and a test suite (\texttt{pytest}, 102 tests). \DIFdelbegin \DIFdel{An archived snapshot of release tag }\texttt{\DIFdel{v1.1-natcomms-revision1}} %DIFAUXCMD
\DIFdel{will be deposited in Zenodo on acceptance, with the persistent DOIinserted at proof stage}\DIFdelend \DIFaddbegin \DIFadd{A permanent mirror of the code archive is deposited on Figshare alongside the data (DOI: }\href{https://doi.org/10.6084/m9.figshare.31653373}{\DIFadd{10.6084/m9.figshare.31653373}}\DIFadd{, CC-BY 4.0)}\DIFaddend .
...
(S10)~\DIFdelbegin \DIFdel{Environment and provenance transparency documentation}\DIFdelend \DIFaddbegin \DIFadd{Experimental coverage matrix: runs per stack, task, and condition}\DIFaddend ;
(S11)~\DIFdelbegin \DIFdel{Revision-task }\DIFdelend \DIFaddbegin \DIFadd{Code, mathematics, and cross-domain task }\DIFaddend details: HumanEval problem identifiers, GSM8K problem identifiers, gpt-4o-2024-11-20 multi-turn extension protocol, and full per-stack per-task EMR with 95\% bootstrap CIs\DIFdelbegin \DIFdel{for revision tasks}\DIFdelend ;
(S12)~LLM-as-judge protocol for the PubMed PM\textsubscript{2.5} probe: judge prompts, three pre-registered criteria (direction, magnitude $\pm$20\%, CI overlap), per-pair verdicts \DIFdelbegin \DIFdel{(5 }\DIFdelend \DIFaddbegin \DIFadd{from two judges (Claude Opus 4.7: 22 }\DIFaddend truly\_contradictory / 3 semantically\_equivalent / \DIFdelbegin \DIFdel{2 ambiguous}\DIFdelend \DIFaddbegin \DIFadd{5 ambiguous; gpt-4o: 27 / 3 / 0; N=30}\DIFaddend ), and cost log\DIFaddbegin \DIFadd{;
(S13)~Per-field reproducibility on the structured-extraction task (per-stack per-field EMR, NED, ROUGE-L, BERTScore)}\DIFaddend .
...
All experimental records (\DIFdelbegin \DIFdel{original }\DIFdelend 4{,}104 runs \DIFdelbegin \DIFdel{plus revision additions for }\DIFdelend \DIFaddbegin \DIFadd{for Tasks~1--4 together with the }\DIFaddend HumanEval, GSM8K, \DIFdelbegin \DIFdel{the multi-turn extension, and the }\DIFdelend \DIFaddbegin \DIFadd{multi-turn-extension, and }\DIFaddend cross-domain PubMed \DIFdelbegin \DIFdel{probe}\DIFdelend \DIFaddbegin \DIFadd{batches}\DIFaddend ), W3C PROV-JSON provenance documents, Run Cards, Prompt Cards, input data with DOIs, and source data for every figure and table are \DIFdelbegin \DIFdel{available to reviewers during peer review via the projectrepository at }\DIFdelend \DIFaddbegin \DIFadd{deposited on Figshare under a CC-BY~4.0 licence (persistent DOI }\href{https://doi.org/10.6084/m9.figshare.31653373}{\DIFadd{10.6084/m9.figshare.31653373}}\DIFadd{) and mirrored at the project's GitHub repository (}\DIFaddend \url{https://github.com/Roverlucas/genai-reproducibility-protocol}\DIFdelbegin \DIFdel{. On acceptance, an archived snapshot of release tag }\DIFdelend \DIFaddbegin \DIFadd{, release tag }\DIFaddend \texttt{v1.1\DIFdelbegin \DIFdel{-natcomms-revision1}\DIFdelend }\DIFdelbegin \DIFdel{will be deposited in Zenodo and a persistent DOI will be minted and inserted at proof stage.
The repository will be made public under a CC-BY~4.0 licence on acceptance.
}\DIFdelend \DIFaddbegin \DIFadd{).
}\DIFaddend 

\item \textbf{Code availability:}
The full source code for the provenance protocol (Run Card, Prompt Card, W3C PROV-JSON generation), the experiment runners (\texttt{run\_experiments.py}, \texttt{run\_claude\_multiturn.py}, \texttt{run\_gemini\_multiturn.py}, \texttt{run\_chat\_control.py}, and \DIFdelbegin \DIFdel{the resumable revision orchestrator}\texttt{\DIFdel{run\_revision\_full.sh}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{a resumable batch orchestrator}\DIFaddend ), the statistical and figure-generation pipeline, and the test suite (102 \texttt{pytest} tests) are \DIFdelbegin \DIFdel{available to reviewers during peer review at }%DIFDELCMD < \url{https://github.com/Roverlucas/genai-reproducibility-protocol}%%%
\DIFdel{. On acceptance, the repository will be made }\DIFdelend publicly available under the MIT licence \DIFdelbegin \DIFdel{and an archived release with a persistent Zenodo DOI will be minted and inserted into this statement at proof stage}\DIFdelend \DIFaddbegin \DIFadd{at }\url{https://github.com/Roverlucas/genai-reproducibility-protocol} \DIFadd{(release tag }\texttt{\DIFadd{v1.1}}\DIFadd{). A permanent mirror of the code archive is deposited on Figshare alongside the data with persistent DOI }\href{https://doi.org/10.6084/m9.figshare.31653373}{\DIFadd{10.6084/m9.figshare.31653373}} \DIFadd{(CC-BY 4.0)}\DIFaddend . A \texttt{README.md} with installation and execution instructions and a pinned \texttt{requirements.txt} (Python~3.14.3) are provided.
\end{itemize}
...
\medskip\noindent\textbf{\DIFdelbegin \DIFdel{Extended Data }\DIFdelend Table~\DIFdelbegin \DIFdel{8.}\DIFdelend \DIFaddbegin \DIFadd{\ref{tab:revision_emr} (below).}\DIFaddend }
Exact match rate per deployment stack on the \DIFdelbegin \DIFdel{revision tasksintroduced during major revision}\DIFdelend \DIFaddbegin \DIFadd{code, mathematics, and cross-domain tasks}\DIFaddend : HumanEval (code generation), GSM8K (math reasoning), the multi-turn refinement extension to gpt-4o and DeepSeek\DIFdelbegin \DIFdel{(R3.3)}\DIFdelend , and the 10-abstract PubMed PM\textsubscript{2.5} cross-domain probe\DIFdelbegin \DIFdel{(R3.5)}\DIFdelend . Cells marked ``---'' indicate stacks not run on that task\DIFdelbegin \DIFdel{in revision}\DIFdelend . 95\% bootstrap CIs from 10{,}000 percentile resamples.

%DIF >  Force arabic numbering and continue from Methods Table 3 (sn-jnl appendices
%DIF >  otherwise resets counter and renames format to \Alph{section}\arabic{table}).
\DIFaddbegin \renewcommand{\thetable}{\arabic{table}}%DIF > 
\setcounter{table}{3}%DIF > 
\DIFaddend \begin{table*}[h]
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
\caption{\DIFdelbeginFL \textbf{\DIFdelFL{Exact match rate (EMR) with 95\% bootstrap CI per deployment stack on the revision tasks.}}
%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Exact match rate (EMR) with 95\% bootstrap CI per deployment stack on the code, mathematics, and cross-domain tasks.}}
\DIFaddendFL T5 = HumanEval (code); T6 = GSM8K (math); T3-ext = multi-turn refinement extended to gpt-4o and DeepSeek; T14 = 10 PubMed PM\textsubscript{2.5} abstracts (Task~2 extraction schema). 10\,000 percentile bootstrap resamples. \DIFaddbeginFL \DIFaddFL{The local--API effect size is reported as Cliff's $\delta$ (last row), pooling per-problem EMRs across the three local stacks versus the four API stacks for each task; $|\delta|{>}0.474$ corresponds to a large effect by Romano }\emph{\DIFaddFL{et~al.}}\DIFaddFL{\mbox{%DIFAUXCMD
...
%DIFDELCMD < \toprule
%DIFDELCMD < \textbf{Stack} & \textbf{T6 GSM8K} & \textbf{T5 HumanEval} & \textbf{T3-ext multi-turn} & \textbf{T14 PubMed PM\textsubscript{2.5}} \\
%DIFDELCMD < \midrule
%DIFDELCMD < API-Claude                & 0.063 [0.03,\,0.10] & 0.393 [0.27,\,0.52] & ---                 & 0.010 [0.00,\,0.03] \\
%DIFDELCMD < API-DeepSeek              & 0.370 [0.26,\,0.49] & 0.837 [0.72,\,0.93] & 0.350 [0.13,\,0.60] & 0.660 [0.46,\,0.85] \\
%DIFDELCMD < API-Gemini                & 1.000 [1.00,\,1.00] & 1.000 [1.00,\,1.00] & ---                 & 0.490 [0.26,\,0.72] \\
%DIFDELCMD < API-GPT4 (gpt-4o)         & 0.267 [0.17,\,0.37] & 0.837 [0.75,\,0.92] & 0.090 [0.02,\,0.16] & 0.420 [0.27,\,0.58] \\
%DIFDELCMD < \midrule
%DIFDELCMD < L-Gemma2                  & 1.000 [1.00,\,1.00] & 1.000 [1.00,\,1.00] & ---                 & 1.000 [1.00,\,1.00] \\
%DIFDELCMD < L-LLaMA3                  & 0.920 [0.85,\,0.97] & 0.947 [0.89,\,0.99] & ---                 & 1.000 [1.00,\,1.00] \\
%DIFDELCMD < L-Mistral                 & 0.840 [0.77,\,0.91] & 0.920 [0.87,\,0.97] & ---                 & 0.960 [0.88,\,1.00] \\
%DIFDELCMD < C-LLaMA3 (Together AI)    & 1.000 [1.00,\,1.00] & 1.000 [1.00,\,1.00] & ---                 & 1.000 [1.00,\,1.00] \\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{tabular}%
%DIFDELCMD < }
%DIFDELCMD < %%%
...
\DIFaddFL{$^{\ast}$Cliff's $\delta$ not computed for T3-ext: this row reports only the gpt-4o and DeepSeek additions. For local-vs-API comparisons on multi-turn refinement, see Fig.~\ref{fig:multiturn} (Task~3, with local stacks Gemma~2 9B, Mistral~7B, and LLaMA~3 8B). Interpretation thresholds following Romano }\emph{\DIFaddFL{et~al.}}\DIFaddFL{\mbox{%DIFAUXCMD
\cite{romano2006appropriate}}\hskip0pt%DIFAUXCMD
...
\emph{Nat. Mach. Intell.} \textbf{8} (2026). Advance online publication.
...
\bibitem{he2025nondeterminism}
He, H. \& Thinking Machines Lab.
Defeating nondeterminism in LLM inference.
Thinking Machines Lab Connectionism Series, \url{https://thinkingmachines.ai/blog/defeating-nondeterminism-in-llm-inference/} (2025).
